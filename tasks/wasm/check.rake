...
#
# Every one compiles `beni-sys`, which links an archive on every target,
# so each depends on Stage B (`beni:build`): the wasm32 lane reads the
# wasi archive, the host lane the host one. The Stage C artifact tasks
...
namespace :wasm do
  ...
  task check: ["beni:build"] do
    ...
    # (it never reads the vendor tree there); the host lane finds its
    # archive through the vendor tree.
    target = KobakoWasm.wasm_target_or_host
    target_flag = target ? ["--target", target] : []
    env = if target
            { "MRUBY_LIB_DIR" => KobakoWasm::MRUBY_LIB_DIR, "WASI_SDK_PATH" => KobakoWasm::WASI_SDK_DIR }
          else
            KobakoWasm::HOST_CARGO_ENV
          end
