...
require_relative "kobako_bench_confirm"
...
class KobakoBenchConfirmTest < Minitest::Test
  def test_unanimous_slowdown_past_threshold_is_a_regression
    rows = KobakoBench::Confirm.judge(pairs(base: 100.0, currents: [94.0, 95.0, 93.0]))
  ...
  def test_mixed_directions_are_noise
    rows = KobakoBench::Confirm.judge(pairs(base: 100.0, currents: [93.0, 107.0, 94.0]))
  ...
  def test_unanimous_but_sub_threshold_mean_is_noise
    rows = KobakoBench::Confirm.judge(pairs(base: 100.0, currents: [99.0, 98.5, 99.2]))
  ...
  def test_unanimous_speedup_past_threshold_is_an_improvement
    rows = KobakoBench::Confirm.judge(pairs(base: 100.0, currents: [105.0, 106.0, 107.0]))
  ...
  def test_wide_pair_spread_is_unstable_even_when_unanimous
    rows = KobakoBench::Confirm.judge(pairs(base: 100.0, currents: [60.0, 95.0, 90.0]))
