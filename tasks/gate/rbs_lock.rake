...
# +rake gate:rbs:lock+ — gate that the gem-sourced RBS pins in
  ...
  namespace :rbs do
    ...
    task :lock do
      ...
      unless drift.empty?
        ...
        header = "gate:rbs:lock: collection lock drifted from Gemfile.lock — run `rbs collection update`:"
        abort "#{header}\n#{rows.join("\n")}"
      end

      puts KobakoReport.gate(name: "gate:rbs:lock", ok_summary: "gem-sourced RBS pins match Gemfile.lock")
    end
  end
end
