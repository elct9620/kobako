...
class TestCodecUnrepresentableGuard < Minitest::Test
  ...
  def test_permissive_object_encodes_as_unsupported_not_nil
    ...
    assert_raises(UnsupportedTypeError,
                  "a permissive method_missing object through Encoder.encode must raise UnsupportedTypeError, " \
  ...
  def test_basic_object_proxy_encodes_as_unsupported
    ...
    assert_raises(UnsupportedTypeError,
                  "a BasicObject proxy through Encoder.encode must raise UnsupportedTypeError") do
  ...
  def test_guard_ext_id_is_rejected_on_decode
    ...
    assert_raises(InvalidTypeError,
                  "an ext frame carrying the guard's packer-only id (0x7F) must be rejected as InvalidTypeError") do
