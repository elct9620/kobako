...
# Coverage for Kobako::Pool construction pre-flight. Pool.new builds no
# Sandbox, so every case here runs without the native ext.
class TestPoolConstruction < Minitest::Test
  ...
  def test_checkout_timeout_must_be_positive_finite_or_nil
  ...
  def test_nil_checkout_timeout_is_valid
    assert_instance_of Kobako::Pool, Kobako::Pool.new(slots: 1, checkout_timeout: nil),
                       "checkout_timeout: nil through Pool.new must construct a Pool"
  end

  # @behavior PL-002
  # Pinned on the public constant — the keyword default consumes it, and a
  # timed behavioral witness would cost the suite a 5-second wait.
  def test_checkout_timeout_defaults_to_five_seconds
    assert_in_delta 5.0, Kobako::Pool::DEFAULT_CHECKOUT_TIMEOUT_SECONDS, 0.0,
                    "Pool.new without checkout_timeout: must bound the wait at 5.0 seconds"
  end
