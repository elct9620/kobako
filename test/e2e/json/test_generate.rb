...
class TestJsonGenerate < Minitest::Test
  ...
  # @behavior JS-018
  def test_generate_emits_compact_json_for_native_values
  ...
  # The input is built from char codes so the assertion does not depend on
  # source-level escaping.
  # @behavior JS-019
  def test_generate_escapes_control_characters
  ...
  # @behavior JS-020
  def test_generate_escaping_round_trips
  ...
  # @behavior JS-021
  def test_symbol_value_and_key_render_as_name
  ...
  # A JSON-native scalar key renders as its string form, as in CRuby.
  # @behavior JS-022
  def test_scalar_key_renders_as_string
  ...
  # A NaN or infinite Float raises GeneratorError, as CRuby does without
  # allow_nan.
  # @behavior JS-025
  def test_nan_and_infinity_raise_generator_error
  ...
  # An Array or Hash key is not a usable JSON key and raises rather than
  # stringify.
  # @behavior JS-024
  def test_non_scalar_key_raises_generator_error
    assert_guest_raises "JSON::GeneratorError", "JSON.generate({ [1] => 2 })"
  end

  # @behavior JS-023
  def test_float_key_renders_as_string
  ...
  # A String carries JSON text, so a non-UTF-8 byte sequence is refused
  # rather than lossily transcoded.
  # @behavior JS-026
  def test_non_utf8_string_raises_generator_error
    assert_guest_raises "JSON::GeneratorError", "JSON.generate(255.chr)"
  end

  # A Symbol reaches JSON as its name, which is JSON text too, so the same
  # rule holds it — a name read through a render would answer the empty
  # string and emit `""` for a symbol the guest never wrote.
  # @behavior JS-027
  def test_non_utf8_symbol_name_raises_generator_error
  ...
  # A value is classified by its native type, not its class identity, so a
  # subclass of a JSON-native type serializes as that native kind.
  # @behavior JS-028 JS-029 JS-030
  def test_native_subclass_serializes_as_native_kind
    assert_equal "[1,2]",
