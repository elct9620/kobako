...
# Host pre-flight coverage for Kobako::Sandbox#run: each malformed call
# raises a standard Ruby exception synchronously, before any guest
# involvement — a minimal ABI fixture stands in for the Guest Binary so
# these cases run without it. Guest-driven #run behavior lives in
# test_run.rb.
class TestSandboxRunPreflight < Minitest::Test
  ...
  def test_target_rejects_double_colon_segmented_name
  ...
  def test_args_must_not_contain_handle
  ...
  def test_kwargs_values_must_not_contain_handle
    handle = Kobako::Handle.restore(1)
  ...
  # @behavior S-097
  def test_kwargs_keys_must_be_symbols
