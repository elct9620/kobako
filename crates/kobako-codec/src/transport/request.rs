...
/// either a bound constant's path (str of the form `"MyService::KV"`,
...
pub enum Target {
    /// A bound constant's path of the form `"MyService::KV"`
    /// (e.g. `"MyService::KV"`).
    Path(String),
...
mod tests {
    ...
    fn request_golden_empty_args_and_kwargs() {
        ...
        let bytes = req.encode().unwrap();
        // Same hex as the Ruby golden test in test/unit/transport/test_request.rb.
        assert_eq!(
            bytes,
            vec![
                0x95, // fixarray 5
