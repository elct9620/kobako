...
# the gate reads: +wall_row+ the +wall_time+ pair sandbox-driven cases
...
class KobakoBenchComparatorTest < Minitest::Test
  ...
  def test_ips_regression_is_positive_when_throughput_drops
    assert_in_delta 15.0, Comparator.regression_pct(:ips, 1000.0, 850.0), 0.001
  end
  ...
  def test_flags_a_gated_case_present_in_the_run_but_absent_from_the_anchor
    ...
    missing = Comparator.gated_absences(current, base, suites: ["demo"])
  ...
  def test_does_not_flag_a_cold_path_row_that_carries_no_gate_metric
    ...
    assert_empty Comparator.gated_absences(current, base, suites: ["demo"]),
