...
#   8d — Near-cap stdout retention. Run a script that fills the
...
def warm_sandbox
  ...
  # The 8c large-payload return (512 KiB String) and the 8d stdout-fill
...
def measure_per_sandbox_cost(runner)
  ...
  record(runner, "8a-rss-baseline", rss_kb: baseline)
  first = warm_sandbox
...
def record_first_sandbox(runner, baseline, after_first)
  record(runner, "8a-rss-after-1-sandbox",
...
def record_growth(runner, target, rss_kb, after_first_kb)
  ...
  record(runner, "8a-rss-after-#{target}-sandboxes",
...
# ---- 8b: per-#run RSS drift ---------------------------------------------

def measure_invocation_drift(runner)
  sandbox = warm_sandbox
  baseline = gc_then_rss
  record(runner, "8b-rss-before-eval-loop", rss_kb: baseline)
  puts format("8b baseline (1 sandbox, warm): rss=%<r>d KB", r: baseline)
...
def record_retention(runner, before, during)
  after = gc_then_rss
  record(runner, "8c-rss-after-gc",
         rss_kb: after, retained_delta_kb: after - before)
  puts format("8c before=%<b>d KB, during=%<d>d KB (peak +%<p>d), after-gc=%<a>d KB (retained %<r>+d)",
              b: before, d: during, p: during - before, a: after, r: after - before)
end

# ---- 8d: near-cap stdout retention --------------------------------------
...
def measure_near_cap_stdout(runner)
  sandbox = warm_sandbox
  before = gc_then_rss
  record(runner, "8d-rss-before-near-cap-stdout", rss_kb: before)
...
def record_near_cap_retention(runner, before, during)
  after = gc_then_rss
  record(runner, "8d-rss-after-near-cap-stdout-gc",
         rss_kb: after, retained_delta_kb: after - before)
  puts format("8d before=%<b>d KB, during=%<d>d KB (peak +%<p>d), after-gc=%<a>d KB (retained %<r>+d)",
              b: before, d: during, p: during - before, a: after, r: after - before)
end
