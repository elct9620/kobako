...
Kobako maintains a regression benchmark suite covering the six performance dimensions [SPEC.md](../SPEC.md) names as release regression gates (startup, Transport round-trip, codec, mruby VM, Catalog::Handles, yield round-trip) plus three characterization suites (multi-thread behaviour, per-Sandbox RSS, `#preload` + `#run` dispatch). The suite perceives performance drift against a fixed reference point — the committed anchor baseline `benchmark/baseline.json` — rather than certifying a portable performance standard; its absolute numbers are meaningful only on hardware comparable to the machine that produced them. Every run is gated against that anchor and per-release runs are archived under `benchmark/results/`. A cumulative +10% regression past the anchor on any of the six gated benchmarks blocks release until a maintainer reviews it or re-blesses the anchor to deliberately accept the new level.
...
This gate perceives drift on comparable hardware; it is not a portable performance standard. `benchmark/baseline.json` is captured on one machine, so running the gate on different hardware flags the hardware delta, not a regression — it is a maintainer-side, same-machine release check, not a guarantee the library certifies to its users.

`rake bench:gate[current,baseline]` compares a run against the committed anchor baseline `benchmark/baseline.json` and exits non-zero so CI fails the release on either issue it reports: a gated case that regressed past the anchor, or a gated case the anchor does not yet cover. With no arguments `current` is the newest file under `benchmark/results/` and the baseline is the anchor. The comparison logic lives in `tasks/support/kobako_bench_gate.rb`; `rake bench:gate_test` runs its unit tests.

The anchor is fixed, not the previous run, so the **+10 %** budget is cumulative: sub-threshold slowdowns accumulate against the same point across releases instead of resetting each release, and the gate trips once their sum clears the floor. A case is flagged only when its regression past the anchor clears **both** that floor **and** a noise band of `2 × √(cv_current² + cv_baseline²)` (the combined coefficient of variation across the two runs). The floor is the conservative backstop; the noise band can only *widen* the bar on high-variance rows, never narrow it below the floor — so the gate never flags more than a bare +10 % rule would. It only *suppresses* flags on demonstrably noisy rows, which is exactly what the false 512 KiB "codec drop" needed.

The anchor moves only by `rake bench:bless[run.json]`, which copies a run over `benchmark/baseline.json`. Re-blessing is the deliberate act of accepting a new performance level: record the accepted shift and why in [What changed vs previous baseline](#what-changed-vs-previous-baseline) in the same commit. A gated case present in a run but missing from the anchor fails the gate until a re-bless records it, so a new gated benchmark cannot enter unmeasured.

**The gate reads the metric each row is actually about.** Rows carrying `wall_time` (sandbox-driven) are judged on `wall_time` — the guest budget, where a slowdown is a larger value. `wall_time` is insensitive to background load on the same machine but not machine-independent: a slower CPU yields a larger `wall_time`, so the anchor stays comparable only on the machine that produced it. Pure host rows (`3a-host-decode-*` / `3a-host-encode-*`) are judged on the median `ips`. The guest-return rows' host wrapper (`1/ips − wall_time`) is GC/allocator-bound on the largest payloads — a fresh 512 KiB Ruby String per iteration makes it swing with host GC timing while the guest compute is flat — so it is **characterization, not a gate signal**. One-shot / cold-path rows carry no dispersion and are skipped.
