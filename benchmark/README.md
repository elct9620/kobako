...
**Pick the metric the gate is actually about.** For the codec dimension, judge the guest codec on the `3a` / `3b` **guest-return rows' `wall_time`** (the guest export portion — machine-load-insensitive) and the host codec on the **pure host-side `3a-host-decode-*` / `3a-host-encode-*` rows**. The guest-return rows' host-side `ips` (`1/ips − wall_time`, the host wrapper) is GC/allocator-bound on the largest payloads and is **characterization, not a gate signal**: a fresh 512 KiB Ruby String allocated per iteration makes that term swing with host GC timing while the guest compute is flat. Gating on it would trip the +10 % threshold on allocator noise rather than a codec change.
