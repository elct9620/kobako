...
Kobako maintains a regression benchmark suite covering the six performance dimensions [SPEC.md](../SPEC.md) names as release quality gates (startup, Transport round-trip, codec, mruby VM, Catalog::Handles, yield round-trip) plus three characterization suites (multi-thread behaviour, per-Sandbox RSS, `#preload` + `#run` dispatch). The committed anchor baseline `benchmark/baseline.json` is the fixed point every run is gated against; per-release runs are archived under `benchmark/results/`. A cumulative +10% regression past the anchor on any of the six gated benchmarks blocks release until reviewed or re-blessed.
