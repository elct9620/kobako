...
**Action**
1. The developer obtains an Extension — an object exposing `name`, `source` (the mruby idiom), an optional `backend` (a `Kobako::Extension::Backend` pairing a constant path with a provider), and `depends_on` — authored in-house or supplied by a gem.
2. Before the first invocation, the developer calls `sandbox.install(extension)` (optionally several, or a splatted array). To keep per-invocation state from leaking, the developer supplies a callable provider so a fresh backend object is resolved each invocation; a shared, read-only backend is supplied as a fixed object instead.
...
| B-07..B-11, B-62 | Service binding registration, including fillable paths defaulting to `Kobako::Unresolved` | [`behavior/registration.md`](docs/behavior/registration.md) |
...
- **Capability Handle contract:** opaque on both sides of the boundary (cannot be dereferenced; the constructor is internal to the Host Gem on the host and blocked on the guest (B-39), so neither side turns a bare integer into a Handle, and the host's `Catalog::Handles` membership — not construction blocking — is the capability gate); host-allocated when a Service returns a non-wire-representable object (B-14, guest→host return path) or when `#run` receives non-wire-representable arguments (B-34, host→guest argument path), except reflective gadget objects (`Binding` / `Method` / `UnboundMethod`), which are never wrapped (B-43); scoped to a single invocation (`Catalog::Handles` reset at every `#eval` or `#run`); ID bounded by `0x7fff_ffff` (allocation beyond raises `Kobako::HandleExhaustedError`, no silent wraparound).
