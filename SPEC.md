...
The per-anchor behavior specifications (Initial State → Operation → Result / Final State) for B-01..B-53 and the Error Scenarios covering E-01..E-48 live in per-aspect files under `docs/behavior/`, listed per grouping below. The decisions below govern those behaviors; consult the linked file for each anchor's full Initial State / Operation / Result.
...
| **Snapshot** | The bundle Runtime returns to Sandbox on a successful `#eval` / `#run`. Carries the raw return bytes and the stdout / stderr bytes with their truncation flags — the observables that exist only when the guest returns. The Sandbox unwraps it into `#stdout`, `#stderr`, and the decoded return value or raised error per the two-step attribution decision. Usage (`wall_time` / `memory_peak`) is not on the Snapshot: Runtime records it on every outcome, so `#usage` (B-35) reads from Runtime and is populated even on the trap path, where no Snapshot is produced. Maps to Ruby class `Kobako::Snapshot`. | No |
...
- **`crates/`** — the host-side Rust crates the ext consumes as path dependencies: `kobako-runtime` (the engine-neutral runtime contract) and `kobako-wasmtime` (the wasmtime driver — owns the Wasm engine lifecycle and implements the host-side import function `__kobako_dispatch`). Both are publishable so a non-Ruby host can build on them.
