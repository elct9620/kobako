...
| **Proxy** | The guest capability module `Kobako::Proxy`. Guest code gains host-forwarding by mixing it in rather than inheriting a base class: a bound Service's constant `extend`s it — class-level calls forward as a Transport Request whose `target` is the constant's path — and `Kobako::Handle` `include`s it, so instance-level calls forward with the instance's Handle ID as `target`. The module supplies the shared `method_missing` / `respond_to_missing?` dispatch seam, deriving each Request's target from the receiver's identity — a `Kobako::Handle` instance by its id, a class by its constant path, refusing any other receiver guest-side (B-59) — holds no per-instance state, and has no host-side counterpart — the host represents a bound Service as a path entry in `Catalog::Services`, not as a class. | No |
...
- **Request shape:** every Request carries exactly five logical fields — `target` (a bound constant's path or Capability Handle reference, distinguishable at the first wire byte), `method` (string), `args` (ordered list, may include Handle references), `kwargs` (Symbol-keyed map; empty kwargs is always present, never absent), and `block_given` (bool indicating the guest call site supplied a block; the block body is never serialized — only the flag travels).
...
- **Yield round-trip:** the host-initiated counterpart of a Request/Response. The Host Gem re-enters the guest synchronously when a Service method invokes its Yielder (B-24); yield round-trips nest strictly within the dispatch frame that produced the Yielder and stack in LIFO order; each frame holds at most one Yielder.
- **YieldResponse envelope:** tag values are:
...
- **ABI version:** the Wire Spec carries a single u32 ABI version, owned by this specification and independent of every package version (the kobako gem, any published crate). The current version is `2`. The Guest Binary reports the version it was built against through the `__kobako_abi_version` export; the Host Gem validates it by equality at Sandbox construction and raises `Kobako::SetupError` on an absent export or a non-equal value (B-40, E-42). Any change to the Wire Contract, the Wire Codec, or the ABI surface increments the version; there is no compatibility range and no negotiation — each side implements exactly one wire shape, the one its ABI version names. Wire-affecting changes are recorded in `CHANGELOG.md` under Breaking Changes.
...
- **Ext code assignments are stable for the life of an ABI version:** `0x00` Symbol (kwargs map keys MUST be this type), `0x01` Capability Handle (`fixext 4`, big-endian u32 ID, cap `0x7fff_ffff`), `0x02` Fault Envelope (embedded msgpack map carrying `type` / `message` / `details`).
- **Envelope framing:**

  | Envelope | Frame |
  |----------|-------|
  | Request | 5-element msgpack array — `target`, `method`, `args`, `kwargs`, `block_given` |
  | Response | 2-element msgpack array — `status`, `value`-or-fault |
  | Result | the value directly (the Outcome tag discriminates) |
  | Panic | msgpack map — keys `origin`, `class`, `message`, `backtrace`, `details` |
  | Outcome | one-byte tag (`0x01` result / `0x02` panic) followed by its payload |
  | YieldResponse | one-byte tag (`0x01` ok / `0x02` break / `0x04` error; `0x03` reserved) followed by an optional payload |
