...
| **Reply** | The answer to one Call, produced by whichever side received it. A dispatch Reply is a Success (`status=0`, value) or Fault (`status=1`, fault envelope) variant; a Yield Reply additionally carries the `break` outcome a Block can produce. One Reply per Call, always — there is no partial or streaming answer. Maps to `Reply` / `YieldReply` in each core-envelope peer (`kobako_codec::envelope`, `kobako_runtime::envelope`), and has no Ruby counterpart — the native side frames every Reply, so the Host Gem's Ruby layer answers a Call with the arm it selected and that arm's payload bytes rather than with a value object. | No |
...
- **Reply shape:** `status` is the envelope and what follows it is the payload of exactly one of two mutually exclusive variants — Success (`status=0`, `value`) or Fault (`status=1`, fault envelope). Success-versus-fault is an envelope decision: a side learns the outcome from `status` without interpreting a payload byte. No partial success, no streaming.
...
- **Fault envelope:** `type` (one of three reserved values — `"runtime"`, `"argument"`, `"undefined"` — stable across releases), `message` (string), `details` (optional structured data carrying caller-actionable diagnostics). Its sole legal wire position is the Reply `status=1` envelope field; a guest→host payload carrying one anywhere is a wire violation the host rejects (E-50). Adding a new `type` requires a coupled host+guest release.
- **Outcome envelope:** per-invocation final result, distinct from a per-dispatch Reply; two variants — Result envelope (serialized last mruby expression) or Panic envelope (`origin` / `class` / `message` / `backtrace` / `details`, where `origin="service"` attributes to `Kobako::ServiceError` and every other value attributes to `Kobako::SandboxError`; `details` is optional structured data carrying caller-actionable diagnostics such as `available` constant lists for E-27). An outcome the core envelope cannot frame, an absent one included, raises `Kobako::TrapError`.
...
- **Envelope framing:**
  ...
  | Panic | `origin` · `class` · `message` byte strings · `backtrace` byte-string list · `details` remainder (empty = absent) |
