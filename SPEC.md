...
- Multi-tenant quota / billing logic, cross-Sandbox fairness scheduling, or cross-invocation aggregate resource metrics (the in-Sandbox per-invocation wall-clock timeout and linear memory cap from [`S-149`](docs/spec/behavior/sandbox.md), [`S-121`](docs/spec/behavior/sandbox.md), the per-invocation usage observability that mirrors those caps from [`S-058`](docs/spec/behavior/sandbox.md), [`S-060`](docs/spec/behavior/sandbox.md), the bounded warm-pool checkout from [`pool.md`](docs/spec/behavior/pool.md), and the per-Sandbox `gvl:` GVL-release mode from [`RT-026`](docs/spec/behavior/runtime.md) are in scope; fairness ordering among waiting checkouts, cross-Sandbox aggregation, and any adaptive or per-invocation scheduling observability are not)
...
| **Wire Spec** | The two-layer contract governing all host↔guest Transport messages — a fixed-layout core envelope carrying an opaque payload; not a runtime object but a shared protocol both sides implement | In scope |
...
- **Proxy** (`Kobako::Proxy`): the guest capability module that gives a constant or object host-forwarding. A Host object bound at a constant-path name via `Sandbox#bind` (a Service) appears to the guest as a constant that `extend`s `Kobako::Proxy`, forwarding its method calls to the host; `Kobako::Handle` `include`s the same module. A multi-segment path (`MyService::KV`) nests the constant under a guest module named by the path prefix; a single-segment path (`File`) binds it at top level.
- **Extension** (`Kobako::Extension`): a guest idiom (mruby `source`) paired with an optional host `backend`, installed on a Sandbox via `#install`. Composes the existing `#preload` (guest source) and `#bind` (host Service) into one setup unit, so guest code sees a native-style constant whose pure methods run in-guest and whose privileged methods dispatch to the backend. kobako defines the Extension contract and the `#install` consumer; it ships no concrete Extension.
...
- Deny the guest every ambient nondeterminism source — wall-clock time, monotonic time, and host entropy — at the Wasm/WASI boundary under the default `hermetic` profile, so guest execution depends only on its source, snippets, and injected Service responses ([`RT-047`](docs/spec/behavior/runtime.md), [`RT-048`](docs/spec/behavior/runtime.md)); the `permissive` profile trades this denial away explicitly ([`RT-049`](docs/spec/behavior/runtime.md), [`RT-050`](docs/spec/behavior/runtime.md))
...
- A `Kobako::Sandbox` may be invoked concurrently from multiple Ruby Threads, on distinct Sandboxes or a shared one; a single Thread runs at most one invocation at a time (see [`RT-001`](docs/spec/behavior/runtime.md), [`RT-057`](docs/spec/behavior/runtime.md)). A host Service object bound once and shared across concurrently-invoking Threads must itself be thread-safe — a `provider:` or `ctx.bind` object is resolved per invocation and carries no such obligation.

**Output guarantees:**
- Every Sandbox invocation (`#eval` or `#run`) either returns a frozen `Kobako::Execution` or raises exactly one of `Kobako::TrapError`, `Kobako::SandboxError`, or `Kobako::ServiceError` — no other outcome is possible. The `Execution` carries that run's `#value` (`#eval`'s last-expression value or `#run`'s entrypoint return value) together with its output captures and usage; a raised error carries the same `Execution` on `#execution` ([`S-065`](docs/spec/behavior/sandbox.md)).
...
- No capability state carries between invocations on the same Sandbox instance — successive or concurrent — regardless of verb; each invocation runs on its own per-invocation state ([`T-038`](docs/spec/behavior/transport-dispatch.md), [`RT-002`](docs/spec/behavior/runtime.md))
- The `kobako` gem name, the public Ruby class names below, and the documented public methods on those classes are stable public contracts:
  ...
  |-----------------------|---------|
  | `Kobako::Sandbox` | `#bind` (with no object it declares a fillable, [`SV-017`](docs/spec/behavior/services.md)), `#install`, `#preload`, `#eval`, `#run` (each accepting an optional `{ \|ctx\| ctx.bind(...) }` per-eval override block, [`SV-024`](docs/spec/behavior/services.md), [`SV-025`](docs/spec/behavior/services.md)); configuration readers `#wasm_path` and `#options` (the `Kobako::SandboxOptions` value object), and the six option readers `#timeout` / `#memory_limit` / `#stdout_limit` / `#stderr_limit` / `#profile` ([`RT-015`](docs/spec/behavior/runtime.md)) / `#gvl` ([`S-003`](docs/spec/behavior/sandbox.md)) that forward to it |
  | `Kobako::Pool` | `.new(slots:, checkout_timeout:, **sandbox_keywords)` — the splat forwards every `Kobako::Sandbox.new` keyword verbatim — with a per-Sandbox setup block; checkout verb `#with` ([`PL-005`](docs/spec/behavior/pool.md), [`PL-008`](docs/spec/behavior/pool.md)) |
  | `Kobako::Execution` | The frozen result of one `#eval` / `#run`, returned on success and carried on a raised invocation-outcome error's `#execution` ([`S-064`](docs/spec/behavior/sandbox.md), [`S-065`](docs/spec/behavior/sandbox.md)). Readers: `#value` and the `#failed?` predicate (`true` iff the run failed); output readers `#stdout` / `#stderr`; truncation predicates `#stdout_truncated?` / `#stderr_truncated?`; usage reader `#usage`. The reusable Sandbox keeps no observables of its own — every capture and usage reader lives here |
  | Error classes | `Kobako::TrapError`, `Kobako::TimeoutError`, `Kobako::MemoryLimitError`, `Kobako::SandboxError`, `Kobako::BytecodeError`, `Kobako::HandleExhaustedError`, `Kobako::ServiceError`, `Kobako::NoServiceError`, `Kobako::ServiceArgumentError`, `Kobako::BlockError`, `Kobako::YieldValueError`, `Kobako::SetupError`, `Kobako::ModuleNotBuiltError`, `Kobako::PoolTimeoutError`; each invocation-outcome error carries the failed run's `Execution` on `#execution` ([`S-065`](docs/spec/behavior/sandbox.md)) |
  ...
  | `Kobako::Context` | The per-invocation object yielded to the `#eval` / `#run` override block. Exposes `#bind(path, object)` to override a declared path's object for that one invocation ([`SV-026`](docs/spec/behavior/services.md), [`SV-027`](docs/spec/behavior/services.md)). Not retained by the Host App — valid only inside the block, and spent once it returns |
  | `Kobako::Unresolved` | The sentinel backing a fillable Service path — `#bind(path)` with no object, sugar for `#bind(path, Kobako::Unresolved)`. It reserves the path's Frame 1 slot so the guest sees the constant while the host defers the object; a guest dispatch to an unfilled fillable fails closed, surfacing as `Kobako::ServiceError` when left unrescued ([`SV-017`](docs/spec/behavior/services.md), [`SV-018`](docs/spec/behavior/services.md)). A single shared constant a Host App may name explicitly at a `bind` site |
  | `Kobako::Extension` | The Extension contract, provided as a value object — `.new(name:, source:, backend:, depends_on:)` with `backend` and `depends_on` optional — together with the nested `Kobako::Extension::Backend` value object, declared as `.new(path:, object:)` (static), `.new(path:, provider:)` (per-invocation), or `.new(path:)` (fillable, defaulting to `Kobako::Unresolved`); `object:` and `provider:` together raise `ArgumentError` ([`EX-013`](docs/spec/behavior/extension.md)). `Sandbox#install` duck-types on the four readers `name` / `source` / `backend` / `depends_on`, so a Host App or gem may pass any conforming object in place of the bundled value type |
...
**Outcome**
The `Worker` snippet replays into every invocation's canonical boot state ([`MR-001`](docs/spec/behavior/mruby.md)), so per-invocation isolation holds ([`S-014`](docs/spec/behavior/sandbox.md)) — no state from request N leaks to request N+1. The host normalizes the `:Worker` Symbol, resolves it on top-level `Object`, and dispatches into `Worker.call(request, opts)`; the return value flows back as an ordinary Ruby value. Backtraces produced inside `Worker.call` are attributed to `(snippet:Worker):line`, giving the developer a clear locator. Errors follow the same three-class taxonomy as J-05; `Kobako::UndefinedEntrypointError` — a `SandboxError` — surfaces when the developer dispatches a name the preload table does not provide, carrying the `#name` that was asked for and the `#available` names it could have been, allowing immediate diagnosis without inspecting the guest source.
...
**Outcome**
Each request holds one pooled Sandbox exclusively for the duration of its block; concurrent requests beyond `slots` wait for a checkin, and a wait past `checkout_timeout` raises `Kobako::PoolTimeoutError` ([`PL-024`](docs/spec/behavior/pool.md)) so the handler can shed load explicitly. Setup cost is paid once per pooled Sandbox, not once per request; per-invocation isolation ([`PL-010`](docs/spec/behavior/pool.md)) plus the per-run `Execution` each invocation returns ([`S-064`](docs/spec/behavior/sandbox.md)) ensure no request observes another request's state or output. A request whose invocation raised `Kobako::TrapError` simply lets the error propagate — the pool discards that Sandbox at checkin and refills the slot on demand ([`PL-013`](docs/spec/behavior/pool.md), [`PL-014`](docs/spec/behavior/pool.md)), so the next checkout never receives an unrecoverable Sandbox.
...
The behaviors live as scenarios (Given → When → Then) in the per-feature files under `docs/spec/behavior/`, listed below; `sumi verify` holds each scenario to a test that claims it. The decisions below govern those behaviors; consult the linked feature for each scenario's full statement.
...
- **Attribution is two-step:** Step 1 — if the Wasm engine reports a trap (including configured-cap traps), raise `Kobako::TrapError` or its named subclass (`Kobako::TimeoutError` per [`OC-032`](docs/spec/behavior/outcome.md), `Kobako::MemoryLimitError` per [`OC-033`](docs/spec/behavior/outcome.md)). Step 2 — otherwise dispatch on the outcome envelope first-byte tag (`0x01` ok, `0x02` panic). Zero-length outcome bytes or unknown tags raise `Kobako::TrapError` as wire-violation fallback.
...
- **Setup-time errors split by trigger:** API-misuse cases are Host App programming errors that raise `ArgumentError` or `TypeError` and bypass the attribution pipeline; content-failure cases — snippet compile, replay, or bytecode structural failures — raise `Kobako::SandboxError` (or its `Kobako::BytecodeError` subclass) with backtrace attribution to the snippet's canonical name when one is available. The assignment lives in the error-class table below.
- **Construction-time setup failures are a separate class:** `Kobako::Sandbox.new` builds the wasm runtime from `wasm_path` before any invocation runs. An absent or unconstructable artifact ([`RT-059`](docs/spec/behavior/runtime.md), [`RT-060`](docs/spec/behavior/runtime.md)), an ABI mismatch ([`RT-007`](docs/spec/behavior/runtime.md)), or a runtime whose declared isolation profile falls below the requested floor ([`RT-012`](docs/spec/behavior/runtime.md)) raises `Kobako::SetupError` — with the `Kobako::ModuleNotBuiltError` subclass for the unbuilt-artifact case ([`OC-048`](docs/spec/behavior/outcome.md)) — rather than a `TrapError`: no Sandbox is produced, so the four-outcome guarantee and the `TrapError` discard-and-recreate recovery contract apply only after construction succeeds.
- **Snippet replay is uniform across verbs:** preloaded snippets ([`S-053`](docs/spec/behavior/sandbox.md)) replay into every invocation's canonical boot state ([`MR-001`](docs/spec/behavior/mruby.md)), whether the invocation is `#eval` (then user source loads) or `#run` (then entrypoint resolution happens). The first invocation seals the snippet table ([`S-087`](docs/spec/behavior/sandbox.md)), parallel to the sealing of Service registration ([`SV-031`](docs/spec/behavior/services.md)).
- **Extension installation composes preload and bind, adding no wire:** `#install` registers an Extension's `source` as a preloaded snippet ([`EX-001`](docs/spec/behavior/extension.md)) and, when the Extension carries a `backend`, binds `backend.path` as a Service ([`EX-002`](docs/spec/behavior/extension.md)) — so the mechanism is entirely host-side setup over the two existing verbs and touches no wire, codec, or Guest Binary surface. The bound path enters Frame 1 at install time and is sealed with the other registries ([`EX-028`](docs/spec/behavior/extension.md)); the object behind it is chosen per invocation by which keyword the backend declares — a fixed `object:` ([`EX-009`](docs/spec/behavior/extension.md)), a per-invocation `provider:` resolved fresh each invocation ([`EX-012`](docs/spec/behavior/extension.md)), or a fillable defaulting to the `Kobako::Unresolved` sentinel ([`EX-014`](docs/spec/behavior/extension.md)). An Extension's `depends_on` set is checked for presence at the seal ([`EX-029`](docs/spec/behavior/extension.md)). `source` is mandatory: a host object with no guest idiom is bound with `#bind` directly, not installed.
- **Isolation is defined by the canonical boot state:** every invocation observes the deterministic post-boot interpreter state of the Guest Binary ([`MR-001`](docs/spec/behavior/mruby.md)) — identical across invocations on either isolation profile, since the state is fixed before any ambient source is consulted, and carrying no artifact of prior invocations ([`S-013`](docs/spec/behavior/sandbox.md)). How that state is produced — booted per invocation or baked into the artifact at build time — is unobservable and left to the implementation.
- **Pool checkout is a pool verb, outside the invocation outcomes:** `Kobako::Pool` hands out exclusively-held warm Sandboxes ([`pool.md`](docs/spec/behavior/pool.md)); a pooled Sandbox satisfies every behavior identically to a directly constructed one. The checkout-wait bound raises `Kobako::PoolTimeoutError` ([`PL-024`](docs/spec/behavior/pool.md)) without touching any Sandbox state, and the pool itself applies the `TrapError` discard-and-recreate recovery contract at checkin ([`PL-013`](docs/spec/behavior/pool.md)).

**Feature groupings.** The behavior scenarios group by feature as follows:

| Cover | Feature |
|-------|---------|
| Sandbox construction, the `#eval` and `#run` invocation lifecycle, output capture, per-run resource accounting via the `Execution`'s `#usage`, and the frozen `Kobako::Execution` returned by `#eval` / `#run` and carried on a failed run's error `#execution` | [`sandbox.md`](docs/spec/behavior/sandbox.md) |
| `#preload` registration (both `code:` and `binary:` forms), snippet-table sealing, and `#run` entrypoint dispatch | [`sandbox.md`](docs/spec/behavior/sandbox.md) |
| Service binding registration, including fillable paths defaulting to `Kobako::Unresolved` and the per-eval `ctx.bind` override block | [`services.md`](docs/spec/behavior/services.md) |
| Extension installation (`#install`) — composition, backend provider resolution (static / per-invocation / fillable), dependency presence | [`extension.md`](docs/spec/behavior/extension.md) |
| Guest-initiated Transport dispatch and the `Catalog::Handles` lifecycle, the `#run` host→guest auto-wrap of non-wire-representable arguments into Capability Handles, guest→host restoration of a returned Capability Handle into its original host object, and the positional / keyword argument partition that follows Ruby 3 call semantics — a brace-less `key: value` is a keyword, an explicit `{...}` Hash literal stays positional | [`transport-dispatch.md`](docs/spec/behavior/transport-dispatch.md) |
| No Handle id reaches an object the invocation did not deliver — an arbitrary integer presented as a Handle resolves to an object the guest already holds or to nothing, because the `Catalog::Handles` table is minted per invocation and discarded whole | [`transport-dispatch.md`](docs/spec/behavior/transport-dispatch.md) |
| Block / Yield re-entry | [`transport-yield.md`](docs/spec/behavior/transport-yield.md) |
| Host-authoritative rejection of Ruby's ambient reflection / eval surface at guest→host dispatch (with a callable allowlist for `Proc` / `Method` targets), the non-wire-representability of reflective gadget objects (`Binding` / `Method` / `UnboundMethod`), and the non-authoritative guest-side mirror of that rejection | [`transport-boundary.md`](docs/spec/behavior/transport-boundary.md) |
| Each bound target's Exposure — the methods it lets the guest call, fixed when the reference to it is made: its own narrowing predicate when it declares one (opaque when that predicate denies every name), otherwise the methods its own class and the object itself define, leaving out what the class inherits, mixes in, or has built into the platform while keeping a record's member readers — composed beneath the reflection floor; guest-side `respond_to?` probing on bound-constant / Handle proxies | [`transport-boundary.md`](docs/spec/behavior/transport-boundary.md) |
| Guest construction: a bound-Service proxy instance is capability-inert, and a `Kobako::Handle` cannot be constructed (`.new` / `.allocate` raise), so an exact instance arises only from the decoder; dispatch derives its target from the receiver's exact identity and refuses any other receiver guest-side; a guest-held `Kobako::Handle` is frozen at mint — the host's per-invocation `Catalog::Handles` membership stays the capability gate | [`transport-boundary.md`](docs/spec/behavior/transport-boundary.md) |
| The host's ABI-version validation of the Guest Binary at construction, the isolation-profile ladder `permissive < hermetic` and the WASI-boundary denial of ambient time and entropy under `hermetic`, concurrent-invocation isolation (distinct or shared Sandbox), and the `gvl:` scheduling mode | [`runtime.md`](docs/spec/behavior/runtime.md) |
| `Kobako::Pool` — construction with forwarded Sandbox keywords and per-Sandbox setup block, `#with` checkout / checkin with blocking wait, and reachability-tied teardown | [`pool.md`](docs/spec/behavior/pool.md) |
| Every invocation begins from the canonical boot state, and a capability gem's invocation of a guest-supplied callback propagates a raise as an ordinary guest exception, never an FFI-boundary Wasm trap | [`mruby.md`](docs/spec/behavior/mruby.md) |
| The guest's output streams and Kernel writers | [`io.md`](docs/spec/behavior/io.md) |
| Guest-side regexp matching as a guest-internal compute capability that projects to wire types when a result crosses the boundary | [`regexp.md`](docs/spec/behavior/regexp.md), [`regexp-string.md`](docs/spec/behavior/regexp-string.md), [`regexp-matchdata.md`](docs/spec/behavior/regexp-matchdata.md) |
| Guest-side JSON parse / generate, and the JSON capability-reference boundary — `parse` cannot forge a Capability Handle, and `generate` refuses a Handle / a bound constant / un-opted object rather than dispatching to the host | [`json.md`](docs/spec/behavior/json.md) |
| Outcome attribution and the error-class tree | [`outcome.md`](docs/spec/behavior/outcome.md) |
| The payload encoding, the codec boundary, and the core envelope | [`payload-encoding.md`](docs/spec/behavior/payload-encoding.md), [`codec.md`](docs/spec/behavior/codec.md), [`envelope.md`](docs/spec/behavior/envelope.md) |
| The User Journeys, walked end to end | [`journeys.md`](docs/spec/behavior/journeys.md) |

Errors split across the invocation-outcome classes, the construction-time `SetupError`, and the pool-checkout `PoolTimeoutError` (all detailed in [`outcome.md`](docs/spec/behavior/outcome.md)):

| Error class | Scenarios |
|-------------|-----------|
| `Kobako::TrapError` | [`OC-022`](docs/spec/behavior/outcome.md), [`OC-023`](docs/spec/behavior/outcome.md), [`OC-028`](docs/spec/behavior/outcome.md) |
| `Kobako::SandboxError` | [`OC-016`](docs/spec/behavior/outcome.md) — raised as the `Kobako::BytecodeError` subclass for bytecode that will not load ([`OC-013`](docs/spec/behavior/outcome.md)), as the `Kobako::UndefinedEntrypointError` subclass for an entrypoint that is not there ([`OC-019`](docs/spec/behavior/outcome.md)), and as the `Kobako::Transport::Error` subclass for a wire violation ([`WP-051`](docs/spec/behavior/payload-encoding.md)) |
| `Kobako::ServiceError` | [`OC-020`](docs/spec/behavior/outcome.md) — raised as the `Kobako::NoServiceError` subclass when a dispatch reaches no Service method ([`T-121`](docs/spec/behavior/transport-boundary.md)) and as the `Kobako::ServiceArgumentError` subclass when its arguments do not fit ([`T-078`](docs/spec/behavior/transport-dispatch.md)) |
| `Kobako::SetupError` | [`OC-017`](docs/spec/behavior/outcome.md) — raised as the `Kobako::ModuleNotBuiltError` subclass for an artifact that has not been built ([`OC-048`](docs/spec/behavior/outcome.md)) |
| `Kobako::PoolTimeoutError` | [`PL-023`](docs/spec/behavior/pool.md), [`PL-024`](docs/spec/behavior/pool.md) |
| Setup-time `TypeError` / `ArgumentError` | the refusals of the setup verbs in [`sandbox.md`](docs/spec/behavior/sandbox.md), [`services.md`](docs/spec/behavior/services.md), [`extension.md`](docs/spec/behavior/extension.md), [`runtime.md`](docs/spec/behavior/runtime.md), and [`pool.md`](docs/spec/behavior/pool.md) |
| Position-determined | a payload position the guest's own codec does not serve raises the class its position names: `NotImplementedError` in a running guest frame, `Kobako::Transport::Error` at a dispatch's answer, `Kobako::SandboxError` where the host reads it ([`CD-026`](docs/spec/behavior/codec.md) onward) |

---

## Refinement

Behavior is cited by its scenario: a feature file under `docs/spec/behavior/` and the scenario id it holds, which `sumi verify` keeps unique and claimed. This document's own `F-xx`, `J-xx`, and `N-x` anchors follow Naming Principle N-8, and the `rake gate:anchors` gate enforces that each is defined once, contiguous, and resolvable. The Regexp and JSON capabilities keep their intent, surface, and non-goals in [`docs/regexp.md`](docs/regexp.md) and [`docs/json.md`](docs/json.md); their behaviors are scenarios like every other feature's.

### Terminology

Concepts are declared in [`docs/spec/glossary.md`](docs/spec/glossary.md), which is where a name is settled, where a name this specification must not use is recorded, and how a reader meets that vocabulary — one file answering all three. Everything below is what a concept becomes in an implementation — the class it maps to, whether it is public, which class a failure raises — and moves to the interface specification as that is written. A term still defined below rather than in the glossary is one not yet migrated, not one governed differently.

The **Host App** is the only role outside this design; the rest are specified here. Documented aliases are permitted only when both names appear together with the canonical relationship explicitly stated.

---

#### Internal Concepts

These are sub-components and runtime concepts internal to kobako. They are not exposed as a public API to the Host App unless explicitly stated. The **Catalog** family spells its registrations as `Services`, `Snippets`, `Extensions`, and `Handles`; the **Transport** family spells its members as `Dispatcher`, `Call`, `Reply`, and `Yielder`, either side of the wire occupying either role. A dispatched Call's `target` resolves to its **Receiver**, the host object the Call's method runs on. The guest-side `Kobako::Proxy` module gives a bound constant or a `Handle` the forwarding behaviour that turns a guest method call into a Call. **Runtime** is the magnus shell over the wasm + mruby execution unit; per the Single-Invocation Slot invariant it holds at most one active Invocation per OS thread.

| Term | Definition | Public? |
|------|-----------|---------|
| **Sandbox** | The reusable configuration unit instantiated by `Kobako::Sandbox`. Owns one `Runtime` and the registries sealed at the first invocation — `Catalog::Services`, `Catalog::Snippets`, `Catalog::Extensions` — and holds no per-invocation state of its own. Exposes two invocation verbs — `#eval(code)` for one-shot mruby source execution ([`S-040`](docs/spec/behavior/sandbox.md), [`S-013`](docs/spec/behavior/sandbox.md)) and `#run(target, *args, **kwargs)` for entrypoint dispatch into a preloaded constant ([`S-045`](docs/spec/behavior/sandbox.md), [`S-047`](docs/spec/behavior/sandbox.md)), each driving one `Context` and returning a frozen `Kobako::Execution` ([`S-064`](docs/spec/behavior/sandbox.md)) — plus the setup verbs `#bind` ([`SV-001`](docs/spec/behavior/services.md), [`SV-017`](docs/spec/behavior/services.md)), `#install` ([`EX-001`](docs/spec/behavior/extension.md), [`EX-002`](docs/spec/behavior/extension.md)), and `#preload` accepting either `code:` plus `name:` for mruby source or `binary:` for RITE bytecode ([`S-052`](docs/spec/behavior/sandbox.md), [`S-055`](docs/spec/behavior/sandbox.md)). Enforces three configurable per-invocation caps — wall-clock timeout, linear memory cap, and per-channel output cap — each independently disableable with `nil`, and carries a per-Sandbox `gvl:` scheduling mode (`:hold` default / `:release`) governing whether an invocation releases the GVL for host-parallel guest execution ([`RT-018`](docs/spec/behavior/runtime.md), [`RT-026`](docs/spec/behavior/runtime.md)). Maps to the Ruby class `Kobako::Sandbox`. | Yes — `Kobako::Sandbox` is stable public API |
| **Context** | The per-invocation driver behind one `#eval` / `#run`. Owns that run's `Catalog::Handles`, its resolved per-invocation Extension backends, and its captures and usage, so no per-invocation state lives on the reusable Sandbox. Resolves each guest→host dispatch target by layering the per-eval `ctx.bind` overrides over this run's provider results over the sealed base bindings ([`SV-021`](docs/spec/behavior/services.md)), then settles the run into its `Kobako::Execution`. Yielded to the optional `#eval` / `#run` override block and spent once that block returns. Maps to the Ruby class `Kobako::Context`. | Partially — `#bind` inside the override block is the stable public surface ([`SV-024`](docs/spec/behavior/services.md)); every other member is internal |
| **Execution** | Returned on success and carried on a failed run's invocation-outcome error via `#execution` ([`S-064`](docs/spec/behavior/sandbox.md), [`S-065`](docs/spec/behavior/sandbox.md)); its `#failed?` predicate is `true` iff the run failed, so a returned and a carried Execution stay distinct even when both `#value` are `nil`. Bundles that run's `#value` (the deserialized guest result), its `#stdout` / `#stderr` captures with truncation predicates ([`S-023`](docs/spec/behavior/sandbox.md), [`S-024`](docs/spec/behavior/sandbox.md)), and its `#usage` ([`S-058`](docs/spec/behavior/sandbox.md), [`S-060`](docs/spec/behavior/sandbox.md)). Maps to the Ruby class `Kobako::Execution`. | Yes — `Kobako::Execution` is stable public API |
| **Pool** | The bounded warm-Sandbox checkout unit instantiated by `Kobako::Pool`. Owns up to `slots` Sandboxes, each constructed lazily with the forwarded `Sandbox.new` keywords and set up once by the per-Sandbox block ([`PL-004`](docs/spec/behavior/pool.md), [`PL-005`](docs/spec/behavior/pool.md)); hands them out one exclusive holder at a time via `#with`, blocking up to `checkout_timeout` when all slots are held ([`PL-011`](docs/spec/behavior/pool.md), [`PL-024`](docs/spec/behavior/pool.md)); releases everything with its own reachability ([`PL-018`](docs/spec/behavior/pool.md)). Maps to the Ruby class `Kobako::Pool`. | Yes — `Kobako::Pool` is stable public API |
| **Extension** | The install unit instantiated by `Kobako::Extension` — a guest idiom paired with an optional host backend, installed via `Sandbox#install` ([`EX-001`](docs/spec/behavior/extension.md), [`EX-002`](docs/spec/behavior/extension.md)). Exposes four readers: `name` (a Symbol matching `/\A[A-Z]\w*\z/`, used as the preloaded snippet's canonical name and as the `depends_on` match key, independent of any bound path), `source` (the mruby idiom, mandatory), `backend` (a value exposing `path` / `object` / `provider`, or absent), and `depends_on` (Symbol names of Extensions that must also be installed, checked for presence at the seal per [`EX-029`](docs/spec/behavior/extension.md)). `#install` duck-types on these readers and composes the Extension from `#preload` (the `source`) and `#bind` (the `backend`). At most one backend per Extension; multi-backend capabilities compose as separate Extensions linked by `depends_on`. Maps to the Ruby class `Kobako::Extension` and, in the Rust SDK, to the `Extension` trait. | Yes — `Kobako::Extension` is stable public API |
| **Extension::Backend** | The host attachment of an Extension — a value pairing `path` (the constant path the backend binds at, single-segment or nested, spelling the guest constant the idiom routes to) with the source of the bound object, declared by explicit keyword: `object:` for a static object fixed for the Sandbox's life, `provider:` for a no-argument callable invoked once per invocation to yield that invocation's object, or neither for a fillable defaulting to `Kobako::Unresolved` until the host supplies the invocation's object. The kind is chosen by keyword, never inferred from whether the value is callable; `object:` and `provider:` together raise `ArgumentError` ([`EX-013`](docs/spec/behavior/extension.md)). Provider identity is resource identity: one `provider:` shared by several Extensions resolves once per invocation to a single shared object. `Kobako::Extension::Backend` is the bundled value type — `#install` duck-types on `path` / `object` / `provider` — and in the Rust SDK the concept is `struct Backend` carrying a `Provider` enum whose `Static` / `PerInvocation` / `Fillable` variants name the three kinds directly. | Yes — nested under `Kobako::Extension` |
| **Runtime** | The magnus shell over the `kobako-wasmtime` driver that drives `data/kobako.wasm` through the engine-neutral `kobako-runtime` contract. Builds the isolation profile requested at construction and declares the built posture ([`RT-016`](docs/spec/behavior/runtime.md)), which Sandbox construction refuses when it falls below the request. Holds at most one active Invocation per OS thread (Single-Invocation Slot invariant; see `### Implementation Standards` § Invariants for the per-thread statics and per-invocation state it licenses). Governed by the Sandbox's `gvl:` mode, an invocation may release the GVL for the span of guest execution and re-acquire it for each guest→host dispatch callback ([`RT-023`](docs/spec/behavior/runtime.md), [`RT-024`](docs/spec/behavior/runtime.md)); the per-thread GVL nesting state lives under the Single-Invocation Slot. Each invocation entry point returns the guest's raw outcome bytes together with that run's output captures and usage; the driving `Context` settles these into the frozen `Kobako::Execution` ([`S-064`](docs/spec/behavior/sandbox.md)) that surfaces `#stdout` / `#stderr` ([`S-036`](docs/spec/behavior/sandbox.md)) and `#usage` ([`S-061`](docs/spec/behavior/sandbox.md)), populated on every outcome the guest reached — value return or trap alike. Receives the dispatch Proc as a per-invocation call argument; the Proc bridges Runtime to the Transport / Catalog layers without Runtime knowing either name. Maps to Ruby class `Kobako::Runtime`. | No |
| **Catalog::Services** | The host-side registry of path→Service bindings, sealed with the Sandbox's other registries at the first invocation ([`SV-031`](docs/spec/behavior/services.md)). Holds the base bindings each invocation's `Context` resolves against, which the Context layers its per-invocation providers and `ctx.bind` overrides on top of. Maps to Ruby class `Kobako::Catalog::Services`. | No |
| **Catalog::Snippets** | The host-side table of `Kobako::Snippet::Source` / `Kobako::Snippet::Binary` entries, holding source (`code:`) and bytecode (`binary:`) preloads in insertion order. Sealed by the first invocation simultaneously with `Catalog::Services` ([`S-087`](docs/spec/behavior/sandbox.md)). Maps to Ruby class `Kobako::Catalog::Snippets`. | No |
| **Catalog::Extensions** | The host-side registry of installed Extensions ([`EX-001`](docs/spec/behavior/extension.md), [`EX-002`](docs/spec/behavior/extension.md)). Composes each Extension onto its sibling registries at install time — the `source` into `Catalog::Snippets`, the `backend.path` into `Catalog::Services` — asserts every declared `depends_on` is present when the Sandbox seals ([`EX-030`](docs/spec/behavior/extension.md)), and resolves each `provider:`-backed path afresh for every invocation, handing that map to the driving `Context` rather than mutating the sealed registry ([`EX-012`](docs/spec/behavior/extension.md)). Maps to Ruby class `Kobako::Catalog::Extensions`. | No |
| **Catalog::Handles** | The host-side table allocating `Kobako::Handle` tokens, mapping Handle IDs to Ruby objects. Minted fresh by each invocation's `Context`, so guest→host dispatch and host→guest auto-wrap share one allocator scoped to that run. The table a run allocates into is never reachable from another run, so every Handle it issued is invalid outside it ([`T-016`](docs/spec/behavior/transport-dispatch.md), [`T-038`](docs/spec/behavior/transport-dispatch.md)). Maps to Ruby class `Kobako::Catalog::Handles`. | No |
| **Proxy** | The guest capability module `Kobako::Proxy`. Guest code gains host-forwarding by mixing it in rather than inheriting a base class: a bound Service's constant `extend`s it — class-level calls forward as a Transport Call whose `target` is the constant's path — and `Kobako::Handle` `include`s it, so instance-level calls forward with the instance's Handle ID as `target`. The module supplies the shared `method_missing` / `respond_to_missing?` dispatch seam, deriving each Call's target from the receiver's identity — a `Kobako::Handle` instance by its id, a class by its constant path, refusing any other receiver guest-side ([`T-113`](docs/spec/behavior/transport-boundary.md)) — holds no per-instance state, and has no host-side counterpart — the host represents a bound Service as a path entry in `Catalog::Services`, not as a class. | No |
| **Handle** | An opaque integer token used on either side of the wire to reference a Ruby object that is not directly wire-representable — a host-side stateful object returned by a Service call (host→guest return path), or a host-side argument passed into `#run` that requires auto-wrap (host→guest argument path). The guest can pass it as a dispatch target or argument in subsequent calls, and invoking methods on it dispatches as a guest→host Transport Call; the guest never sees the underlying Ruby value. Maps to two independent implementations with the same canonical name `Kobako::Handle`: the Ruby class runs in the host process; the mruby class runs inside the Wasm guest as a `Kobako::Proxy`-including class, its instances carrying the Handle ID and forwarding method calls to the host as Transport Calls whose `target` is that ID. They share neither code nor instances. | Partially — `Kobako::Handle` instances may surface as fields on raised `SandboxError` or `ServiceError` instances; the Host Gem holds the only constructor (no public `.new`) and Handles enter Host App code only through error fields |
| **Capability Handle** | A Handle that represents a Ruby object outside the payload codec's type mapping — either a stateful resource (session, connection, any object the codec cannot transmit by value) returned from a Service call, or a non-wire-representable value (`StringIO`, a custom Env / Context object) supplied as a `#run` argument. Rides the core envelope's `target` field as a bare ID, and a payload position as the codec's Handle representation (MessagePack ext type `0x01`). "Capability Handle" is used when emphasizing the capability-granting semantics; "Handle" is used for brevity elsewhere — both refer to the same concept. | No — same visibility as Handle; no distinct class exists |
| **Call** | A message inviting the other side to run one method and answer. Carries `target`, `method`, the invocation payload, and `block_given`. Either side may issue one: the guest issues a Call to reach a bound Service or a Handle; the host issues a Call to re-enter a Block it received (the Yield Call, which carries only the yield arguments). Maps to the Ruby class `Kobako::Transport::Call` and to `kobako_transport::envelope::Call`. | No |
| **Reply** | The answer to one Call, produced by whichever side received it. A dispatch Reply is a Success (`tag=0`, value) or Fault (`tag=1`, fault body) variant; a Yield Reply additionally carries the `break` outcome a Block can produce. One Reply per Call, always — there is no partial or streaming answer. Maps to `kobako_transport::envelope::Reply` / `YieldReply`, and has no Ruby counterpart — the native side frames every Reply, so the Host Gem's Ruby layer answers a Call with the arm it selected and that arm's payload bytes rather than with a value object. | No |
| **Receiver** | The host object a dispatch resolves its Call `target` to — the object the Call's `method` runs on. A constant-path target resolves to the Service bound at that path; a Handle target resolves to the object its ID references in `Catalog::Handles`. Both target kinds resolve to a Receiver under the same dispatch constraints — the reflection floor ([`T-117`](docs/spec/behavior/transport-boundary.md)) and the Receiver's Exposure ([`T-125`](docs/spec/behavior/transport-boundary.md)) bind bound-constant and Handle receivers identically; a Receiver whose Exposure permits no name is opaque: the guest holds it, passes it, and returns it, but can call nothing on it. Names the role an ordinary host object occupies during a dispatch — no class of its own exists on either side. | No |
| **Exposure** | The methods a Receiver lets the guest call, fixed when the reference to it is made — a binding at `bind`, `install`, per-invocation provider resolution, or `ctx.bind`, and a Capability Handle at allocation. A Receiver declaring its own narrowing predicate is asked on every dispatch, so a name it answers dynamically stays reachable; any other Receiver's Exposure is derived from what its own class and the object itself define as they stood then, and a method defined later is not reachable through that reference. Maps to the Ruby class `Kobako::Transport::Exposure`; the Rust frontend needs none, since a `Receiver`'s own `respond_to_guest` already is its Exposure. | No |
| **Block** | A mruby block (or Proc/lambda) the guest passes alongside a Service method call. The block body lives inside the Guest Binary and is never serialized; only its presence is signalled on the wire (Call `block_given` field). Scoped to the single dispatch call that received it — not reusable after that dispatch returns. | No — surfaces only as the `&block` argument the Service method receives |
| **Yield** | A single synchronous round-trip from a Service method into the Block it received — a host-issued Call answered by a guest Yield Reply. The host Service method invokes `yield` or `block.call` on its block argument; the Host Gem re-enters the Guest Binary, executes the block body, and returns the block's result to the host yield site. Each `yield` is an independent round-trip; a Service method may yield zero or more times during a single dispatch. | No |
| **Yielder** | The host-side object the Host Gem materialises to represent the guest block to the Service method. It rides the `&block` slot, so the Service method observes it as an ordinary Ruby Proc with loose arity (matches `&block` conventions) and invokes it via `yield` or `block.call`; each invocation is a synchronous round-trip into the guest ([`T-085`](docs/spec/behavior/transport-yield.md)). Valid only for the duration of the dispatch that produced it; invocation after that dispatch frame returns raises ([`T-136`](docs/spec/behavior/transport-yield.md)). Maps to Ruby class `Kobako::Transport::Yielder`. | No |
| **Guest** | The guest ABI contract a Guest Binary implements — the entry methods the six exports call, emitted with `export_guest!`. A guest that implements it directly is indistinguishable to the host from the bundled one; mruby is one implementation, not the contract. Maps to `kobako_core::Guest`. | Yes — third parties implement it (N-9) |
| **MrbGuest** | The mruby harness a shell assembles its guest from: the required `init_gems` hook naming the `beni::Gem` set, the `Codec` associated type naming the payload codec, and provided `eval` / `run` / `yield_to_block` flows a shell inherits or replaces. Maps to `kobako_mruby::MrbGuest`. | Yes — third parties implement it (N-9) |
| **PayloadCodec** | The obligations a payload codec fills — the payload positions of a Call, a Run, a Reply's ok arm, a Yield Reply, and an Outcome. A guest names its choice on `MrbGuest::Codec`; a Rust host names it by building the SDK without its `msgpack` feature, leaving every payload position byte-level. The Ruby frontend has no such seam and speaks the default codec. Maps to `kobako_mruby::PayloadCodec`. | Yes — third parties implement it (N-9) |
| **Rust host SDK** | The bare-name `kobako` crate: a second frontend over the same driver, with `Sandbox`, `Options`, `Execution`, `Receiver`, `Fault`, `Yielder`, `Handles`, and `Extension` as its surface. Behaviour is pinned to the Ruby frontend by the differential parity harness; the API shape stays idiomatic Rust rather than mirroring `lib/`. | Yes — stable public API (N-9) |

The grades above, and the obligations each replaceable interface carries, are declared in [`docs/customization.md`](docs/customization.md). The engine contract (`Runtime`, `Snapshot`, `Completion`, `Trap`, `Profile`) is a customization point too, but its vocabulary stays in the `kobako-runtime` crate doc rather than here.
...
|------|-----------|--------------------|----|
| **TrapError** | `Kobako::TrapError` | Wasm engine layer | The Wasm execution engine crashed (OOM, `unreachable`, stack overflow, import signature mismatch) or the wire layer detected a structural violation indicating a corrupted guest runtime (an outcome the core envelope cannot frame, an absent one included) |
...
| **ServiceError** | `Kobako::ServiceError` | Service / capability layer | The guest ran to completion, the mruby script itself did not crash, but a Service capability call reported an application-level failure that was not rescued within the script. Its named subclasses separate the call that never reached a Service method from the one that did — [`transport-dispatch.md`](docs/spec/behavior/transport-dispatch.md) and [`outcome.md`](docs/spec/behavior/outcome.md) map every dispatch failure to the class it raises on each side |
...
| Term | Ruby Class | Layer it represents | When raised |
|------|-----------|--------------------|----|
| **SetupError** | `Kobako::SetupError` | Construction / runtime-setup layer | `Sandbox.new` could not construct the wasm runtime from the configured `wasm_path` — an absent or unreadable artifact, bytes that are not a valid Wasm module, engine / linker / instantiation setup failure, or a runtime whose declared isolation profile falls below the requested floor ([`RT-012`](docs/spec/behavior/runtime.md)). No Sandbox instance is produced, so the four-outcome guarantee and the `TrapError` discard-and-recreate recovery contract do not apply |

**Named subclasses (stable public API):**

| Term | Ruby Class | Superclass | Meaning |
|------|-----------|-----------|---------|
| **TimeoutError** | `Kobako::TimeoutError` | `Kobako::TrapError` | Absolute wall-clock time since the invocation entry (`#eval` or `#run`) reached the configured per-invocation `timeout` (default 60 s); trap fires at the next guest wasm safepoint after the deadline. See [`OC-032`](docs/spec/behavior/outcome.md); [`sandbox.md`](docs/spec/behavior/sandbox.md) covers host-callback accounting. |
| **MemoryLimitError** | `Kobako::MemoryLimitError` | `Kobako::TrapError` | Cumulative guest `memory.grow` since invocation entry would push past the configured `memory_limit` (default 1 MiB) — the mruby image's initial allocation and prior invocations' watermark are folded into the per-invocation baseline rather than the budget; see [`OC-033`](docs/spec/behavior/outcome.md) |
| **HandleExhaustedError** | `Kobako::HandleExhaustedError` | `Kobako::SandboxError` | Handle ID counter in `Catalog::Handles` reached `0x7fff_ffff` (2³¹ − 1) within a single invocation; further allocation is impossible |
| **BytecodeError** | `Kobako::BytecodeError` | `Kobako::SandboxError` | `#preload(binary:)` bytecode failed structural validation during the first invocation's snippet replay — RITE version mismatch ([`S-148`](docs/spec/behavior/sandbox.md)) or corrupt body ([`S-089`](docs/spec/behavior/sandbox.md)). Backtrace attributes to the bytecode's `debug_info` filename when the bytecode carries one. |
| **UndefinedEntrypointError** | `Kobako::UndefinedEntrypointError` | `Kobako::SandboxError` | A `#run` target names no top-level constant ([`S-151`](docs/spec/behavior/sandbox.md)). Carries `#name`, the entrypoint asked for, and `#available`, the top-level constants the preloaded snippets contributed — so the caller corrects the name from the error rather than by inspecting the guest source. |
| **NoServiceError** | `Kobako::NoServiceError` | `Kobako::ServiceError` | A dispatch reached no Service method: the bound path holds nothing ([`T-139`](docs/spec/behavior/transport-dispatch.md)), the Capability Handle is not live in this invocation ([`T-038`](docs/spec/behavior/transport-dispatch.md)), or the method is absent ([`T-121`](docs/spec/behavior/transport-boundary.md)) or outside the guest surface ([`T-193`](docs/spec/behavior/transport-boundary.md), [`T-197`](docs/spec/behavior/transport-boundary.md)). The causes stay indistinguishable, so what a Host App learns is that this call will not succeed by being retried. |
| **ServiceArgumentError** | `Kobako::ServiceArgumentError` | `Kobako::ServiceError` | A dispatch reached the Service method but its arguments did not fit — an unknown keyword, or an arity mismatch ([`T-144`](docs/spec/behavior/transport-dispatch.md), [`T-141`](docs/spec/behavior/transport-dispatch.md)). |
| **BlockError** | `Kobako::BlockError` | `Kobako::Error` | Raised at a Service method's `yield` site when the guest block it yielded to failed ([`T-094`](docs/spec/behavior/transport-yield.md)), carrying the guest-side class on `#klass`. It is not an invocation outcome and carries no `Execution`: a Service rescues it, or it returns to the guest, which continues the exception it raised. |
| **YieldValueError** | `Kobako::YieldValueError` | `Kobako::Error` | Raised at a Service method's `yield` site when a yield argument has no wire representation, so the round-trip never left the host and the block never ran ([`T-154`](docs/spec/behavior/transport-yield.md), [`T-156`](docs/spec/behavior/transport-yield.md)). Like `BlockError` it is not an invocation outcome and carries no `Execution`: a Service rescues it, or the dispatch answers as that Service having failed. |
| **ModuleNotBuiltError** | `Kobako::ModuleNotBuiltError` | `Kobako::SetupError` | The Guest Binary artifact is absent at the resolved `wasm_path` — the common state on a fresh clone before `rake compile`. See [`RT-059`](docs/spec/behavior/runtime.md) |

**Wire-level error class:** The string `"Kobako::Transport::Error"` appears as the `name` field value in a Panic envelope (defined in [`docs/wire-contract.md`](docs/wire-contract.md) § Outcome Envelope; the governing summary lives below in `### Wire Contract`) to signal that the Transport layer detected a wire violation, or that a dispatch failed with no Service outcome to report ([`T-163`](docs/spec/behavior/transport-dispatch.md), [`T-046`](docs/spec/behavior/transport-dispatch.md)). On the host side this maps to a raised `Kobako::Transport::Error`, a `Kobako::SandboxError` subclass — so a single `rescue Kobako::SandboxError` still catches it, while callers that want to single out these paths from ordinary script failures can `rescue Kobako::Transport::Error` directly. (The guest mruby class `Kobako::Transport::Error` exists only to be raised inside the Guest Binary; it is captured by the guest's top-level handler and converted into the panic envelope string.)
...
The abstract logical shape of every host↔guest message exchanged during a `#run` invocation is specified in detail in [`docs/wire-contract.md`](docs/wire-contract.md). It is a Consistency-layer contract: both sides implement it independently, and a kobako gem release ships exactly one version of it. Byte-level encoding is specified across the two layers [`docs/wire-codec.md`](docs/wire-codec.md) anchors — the fixed-layout core envelope and the payload codec. The decisions below govern the contract; the linked documents reproduce them as field tables, envelope shapes, and per-position rules.

- **Transport role:** every exchange is one Call answered by one Reply, and both directions use that same pair. The Guest Binary opens every conversation; the Host Gem answers synchronously within the same Wasm import call frame, and issues a Call of its own only to re-enter a Block, nested inside the dispatch it is still answering. There are no callbacks, promises, or yield-resume mechanisms.
- **Call shape:** a Call separates the fields that route it from the fields that feed the method it routes to. The **envelope** carries `target` (a bound constant's path or Capability Handle reference, distinguishable at the first wire byte), `method` (string), and `block_given` (bool indicating the guest call site supplied a block; the block body is never serialized — only the flag travels). The **payload** carries `args` (ordered list, may include Handle references) and `kwargs` (Symbol-keyed map; empty kwargs is always present, never absent). A side routes a Call — resolving the target, naming the method, knowing whether a block is coming — without interpreting a payload byte. A Yield Call carries a payload only; the dispatch frame it nests in supplies its routing.
- **Reply shape:** `tag` is the envelope and what follows it is exactly one of two mutually exclusive variants — Success (`tag=0`, the value as payload) or Fault (`tag=1`, a Fault, itself an envelope shape). Success-versus-fault is an envelope decision: a side learns the outcome from `tag` without interpreting a payload byte. No partial success, no streaming.
- **Capability Handle contract:** opaque on both sides of the boundary (cannot be dereferenced; the constructor is internal to the Host Gem on the host and blocked on the guest ([`T-110`](docs/spec/behavior/transport-boundary.md)), so neither side turns a bare integer into a Handle, and the host's `Catalog::Handles` membership — not construction blocking — is the capability gate); host-allocated when a Service returns a non-wire-representable object ([`T-001`](docs/spec/behavior/transport-dispatch.md), guest→host return path) or when `#run` receives non-wire-representable arguments ([`T-066`](docs/spec/behavior/transport-dispatch.md), host→guest argument path), except reflective gadget objects (`Binding` / `Method` / `UnboundMethod`), which are never wrapped ([`T-014`](docs/spec/behavior/transport-dispatch.md)); scoped to a single invocation (each `#eval` / `#run` mints its own `Catalog::Handles`); ID bounded by `0x7fff_ffff` (allocation beyond raises `Kobako::HandleExhaustedError`, no silent wraparound).
- **Fault:** `type` (a reserved value naming which failure it reports — `"runtime"`, `"argument"`, `"undefined"`, `"internal"`, `"block"`, each stable across releases) and `message` (string), and nothing else: a Fault travels host→guest, so it carries only what its author can keep bounded — which is also why a guest's own block failure comes back as a category rather than as host-supplied detail ([`T-184`](docs/spec/behavior/transport-yield.md)). It is the whole of a Reply's fault body (`tag=1`) and rides the core envelope rather than the payload, so a guest reads a refusal without reaching any codec. The set is open and additive: a receiver reads a `type` it predates as `"undefined"` and skips a Fault field it predates, so an addition costs no ABI version increment while a change to what an existing value or field *means* does.
- **Outcome envelope:** per-invocation final result, distinct from a per-dispatch Reply; two variants — an ok Outcome (serialized last mruby expression) or a Panic Outcome (`origin` / `name` / `message` / `backtrace` / `available`, where `origin="service"` attributes to `Kobako::ServiceError` and every other value attributes to `Kobako::SandboxError`; `available` lists the names the invocation could have used in place of the one it named, empty when the failure offers no correction). Every Panic field is typed at the core envelope, so a Panic carries no codec-encoded content and attribution never depends on one. An outcome the core envelope cannot frame, an absent one included, raises `Kobako::TrapError`.
- **Yield round-trip:** the reverse-direction Call/Reply pair. The Host Gem re-enters the guest synchronously when a Service method invokes its Yielder ([`T-085`](docs/spec/behavior/transport-yield.md)); yield round-trips nest strictly within the dispatch frame that produced the Yielder and stack in LIFO order; each frame holds at most one Yielder. A yield argument with no wire representation fails at the Service's yield site before the guest is re-entered ([`T-156`](docs/spec/behavior/transport-yield.md)) — the mirror of the guest's own refusal at its dispatch call site ([`T-148`](docs/spec/behavior/transport-dispatch.md)).
- **Yield Reply envelope:** tag values are:

  | Tag | Meaning |
  |-----|---------|
  | `0x01` ok | the block returned a wire-legal value |
  | `0x02` break | the block executed `break val` from a non-lambda context; unwinds the Service method per [`T-089`](docs/spec/behavior/transport-yield.md) |
  | `0x04` error | the block raised, returned an unrepresentable value, used `return` from a non-lambda block, or invoked an escaped Yielder ([`transport-yield.md`](docs/spec/behavior/transport-yield.md)) |
  | `0x03` | reserved — rejected as a wire violation |
- **ABI version:** the Wire Spec carries a single u32 ABI version, owned by this specification and independent of every package version (the kobako gem, any published crate). The current version is `3`. The Guest Binary reports the version it was built against through the `__kobako_abi_version` export; the Host Gem validates it by equality at Sandbox construction and raises `Kobako::SetupError` on an absent export or a non-equal value ([`RT-006`](docs/spec/behavior/runtime.md), [`RT-007`](docs/spec/behavior/runtime.md)). Any change to the Wire Contract, the core envelope layout, or the ABI surface increments the version; there is no compatibility range and no negotiation — each side implements exactly one wire shape, the one its ABI version names. Choosing a different payload codec is not such a change: the codec is an agreement between two endpoints, outside the versioned surface. Wire-affecting changes are recorded in `CHANGELOG.md` under Breaking Changes.
...
- **Two encoding layers:** the **core envelope** is a fixed layout — length-prefixed byte strings, `u8` tags, big-endian `u32` — carrying only routing and outcome attribution, and it is not MessagePack. The **payload** it hands through is an opaque byte string a payload codec owns. Byte-level references: [`docs/wire/envelope.md`](docs/wire/envelope.md) and [`docs/wire/payload-msgpack.md`](docs/wire/payload-msgpack.md).
- **The fixed tier is one crate.** A host, a payload codec, and a guest are chosen independently only because the core envelope and the ABI's fixed values are the same in every assembly. `kobako-transport` owns both — the envelope layout, the ABI version, the packed-return and frame-prefix layouts, and the single-dispatch size cap — and owns nothing else: it depends on no crate, and every other tier depends on it. An engine, a codec, and a guest are each replaceable against it; it is replaceable against none of them. Each of those values has exactly one definition, so two assemblies cannot come to disagree about one. The exports and the host import that carry the ABI are emitted by the guest contract crate and implemented by the driver, against the values this tier fixes.
- **MessagePack is the default payload codec, not the wire's only codec.** The bundled Guest Binary and the Ruby frontend speak it, and a host that chooses nothing gets it. Two endpoints that agree on another schema replace it, and the core envelope and the ABI surface are unchanged by that substitution, so it is not an ABI version increment. The dependency goes with it completely on either endpoint: a guest that replaces it carries no MessagePack at all, and neither does a host — whether built directly on the fixed tier and the driver, or on the Rust SDK with its `msgpack` feature off.
- **The two decodes are separable.** Decoding an envelope requires nothing from the codec, and decoding a payload requires nothing from the envelope beyond its bytes and length. A frontend may therefore decode the two on different sides of its own internal boundaries, and an endpoint that only routes messages needs no codec at all.
- **A replacement codec's obligations** are the positions it fills, not the encoding it uses. Carrying one value is the floor — every invocation ends by writing an Outcome — and the remaining positions are capabilities the codec chooses; a position it does not serve refuses there, distinguishably from a message it could not read. The Call payload is the one position that must express positional and keyword arguments distinguishably, because it is the only one that carries keywords. Handles ride payload positions only if the codec has a representation for them — a Handle `target` is an envelope field and stays reachable regardless. A Reply's fault body is not a codec position: a Fault is kobako's own data and rides the envelope, so a replacement schema neither encodes nor reads one. The obligations, and the seam each endpoint names its choice at, are specified in [`docs/customization.md`](docs/customization.md).
- **The Ruby frontend is fixed to the default codec.** MessagePack is Ruby's native choice and the gem speaks it directly, with no seam to substitute at; the guest and the Rust host SDK are where a replacement codec is installed.
- **Envelope framing:**

  Every field but the last is self-delimiting; the last consumes the remainder, whose extent the transport already supplies. Byte strings carry a big-endian `u32` length; lists carry a `u32` count.

  | Envelope | Frame |
  |----------|-------|
  | Call | `kind` u8 · `target` (byte string, or u32 Handle ID when `kind=1`) · `method` byte string · `block_given` u8 · `payload` remainder |
  | Reply | `tag` u8 (`0` ok / `1` fault) · body remainder — the value, or a Fault |
  | Fault | `kind` u8 (`0` runtime / `1` argument / `2` undefined) · `message` byte string |
  | Outcome | `tag` u8 (`0x01` ok / `0x02` panic) · body remainder — the value alone, or a Panic |
  | Panic | `origin` byte string · an Error Record · `available` byte-string list — the same record the Yield Reply error arm carries, so the two failure channels share one shape |
  | Run | `entrypoint` byte string · `payload` remainder |
  | Yield Call | the payload alone, framed by the ABI's `req_len` |
  | Yield Reply | `tag` u8 (`0x01` ok / `0x02` break / `0x04` error; `0x03` reserved) · body remainder — the value, or an Error Record |
  | Error Record | `name` · `message` byte strings · `backtrace` byte-string list |
  | Frame 1 | byte-string list of bound constant paths |
  | Frame 3 | `count` u32, then per entry `kind` u8 (`0` source / `1` bytecode) · `name` byte string (source only) · `body` byte string |
- **Payload type set (MessagePack codec):** exactly 11 entries — `nil`, `bool`, `int`, `float`, `str`, `bin`, `array`, `map`, the general `ext` channel, and the two ext codes dispatched through it, `0x00` (Symbol) and `0x01` (Capability Handle). Any msgpack type or ext code outside this set is a wire violation; both sides reject without attempting to decode. Ext code assignments are stable for the life of an ABI version: `0x00` Symbol (kwargs map keys MUST be this type), `0x01` Capability Handle (`fixext 4`, big-endian u32 ID, cap `0x7fff_ffff`).
- **Call and Run payloads are a 2-element array** — `args` then `kwargs`, both always present. The positional-versus-keyword partition is a codec concern: a schema without Ruby's call semantics carries its own shape and the envelope is unchanged.
- **ABI surface:** the wire ABI is a closed enumerated set — one host import (`__kobako_dispatch`) and exactly six guest exports (`__kobako_eval`, `__kobako_run`, `__kobako_alloc`, `__kobako_take_outcome`, `__kobako_yield_to_block`, `__kobako_abi_version`). Function names and Wasm signatures are fixed across an ABI version; growth requires a new SPEC anchor that lifts the enumeration. `__kobako_dispatch`, `__kobako_take_outcome`, and `__kobako_yield_to_block` return a packed i64 carrying `ptr` (high 32 bits) and `len` (low 32 bits). `__kobako_eval` and `__kobako_run` are the two invocation entry points; both write a single Outcome envelope to OUTCOME_BUFFER before returning, and the host applies the two-step attribution decision to whichever export it called. `__kobako_abi_version` returns the u32 ABI version the Guest Binary was built against (see `### Wire Contract` § ABI version).
...
| N-8 | `F-xx`, `J-xx`, and `N-x` anchors are assigned monotonically and append-only within `SPEC.md`; existing anchors are never renumbered, and a new entry takes the next integer above the highest of its family. A number freed by a retired anchor is a permanent tombstone — it is never reassigned, so a historical reference to a retired number never rebinds to unrelated content | Feature List, User Journeys, and Naming Principles entries |
...
- **Wire evolves only by ABI version bump** — see `### Wire Contract` § ABI version for the governing statement. Design implication: never add a wire field gated on a flag or negotiated at runtime; a wire change is a new ABI version, and the Wire Spec at any one version is a single coupled artifact.
...
| `Execution#usage` reports its run's `wall_time` and `memory_peak`, sharing its accounting boundary with the matching caps from [`S-149`](docs/spec/behavior/sandbox.md), [`S-121`](docs/spec/behavior/sandbox.md) and populated regardless of outcome ([`S-061`](docs/spec/behavior/sandbox.md), [`S-069`](docs/spec/behavior/sandbox.md)); `memory_peak` never exceeds `memory_limit` on `MemoryLimitError` | Host Gem | Runtime |
| `Sandbox#eval`'s `Execution#value` is the last mruby expression and `Sandbox#run`'s is the entrypoint's `#call` value, both carried via the Outcome's ok arm. A value the guest hands across the boundary that has no wire representation is rejected rather than coerced — no path substitutes an implicit `inspect` / `to_h` / `to_s` string: an invocation return takes the Panic arm instead ([`S-135`](docs/spec/behavior/sandbox.md)), a yield-block result fails the yield round-trip ([`T-135`](docs/spec/behavior/transport-yield.md)), and a dispatch argument or kwargs value fails at the dispatch call site ([`T-148`](docs/spec/behavior/transport-dispatch.md)) | Guest Binary, Wire Spec | Test-time |
...
| Under the default `hermetic` profile the guest's WASI ambient sources are denied: `wasi:clocks` (wall and monotonic) is frozen and `wasi:random` is a constant stream, so no real time or host entropy reaches guest code; under `permissive` the same sources read the host's live clocks and entropy ([`RT-047`](docs/spec/behavior/runtime.md) / [`RT-049`](docs/spec/behavior/runtime.md)). The per-invocation `timeout` is measured on the host clock and enforced by the engine on both rungs, independent of the guest's `wasi:clocks` | Runtime | Runtime |
...
| Single-Invocation Slot: Runtime holds at most one active Invocation per OS thread for the duration of any invocation (`#eval` or `#run`). Nested host→guest dispatch (Service calls Service via a yielded block which calls another Service) shares the same Invocation; nested dispatch frames stack within it ([`T-087`](docs/spec/behavior/transport-yield.md), [`T-092`](docs/spec/behavior/transport-yield.md)). There is no stack of Invocations — at most one per thread. The slot licenses the guest-side per-thread statics (`MRB` slot, `BLOCK_STACK`, `OUTCOME_BUFFER`), the host-side per-invocation state (active caller pointer, deadline, wall-clock entry, memory peak, captures), and the per-thread GVL nesting state that governs release ([`RT-024`](docs/spec/behavior/runtime.md)) | Runtime, Host Gem | Runtime |
| The `gvl:` mode is per-Sandbox and fixed at construction: under `:release` an invocation releases Ruby's GVL only for the span of guest execution and re-acquires it for each guest→host dispatch callback; `:hold`, the default, holds the GVL throughout. Releasing changes scheduling only — every isolation ([`RT-004`](docs/spec/behavior/runtime.md)), Handle-lifecycle ([`runtime.md`](docs/spec/behavior/runtime.md)), capture ([`RT-025`](docs/spec/behavior/runtime.md)), and outcome behavior holds identically, whether concurrent Threads use distinct Sandboxes or share one ([`RT-030`](docs/spec/behavior/runtime.md), [`RT-031`](docs/spec/behavior/runtime.md)) | Runtime, Host Gem | Runtime |
| The wasmtime driver crate (`kobako-wasmtime`) declares no `magnus` dependency — only `ext/kobako` links `magnus`. The GVL-released span ([`RT-026`](docs/spec/behavior/runtime.md)) calls into this driver, so the absent declaration is what keeps a Ruby VALUE out of the span's reach by construction; a build gate rejects the declaration in the driver's manifest | Architecture, Host Gem | Build-time |
| `Kobako::Sandbox#preload` accepts `code:` plus `name:` (matching `/\A[A-Z]\w*\z/`) for source, or `binary:` alone for RITE bytecode treated as opaque bytes whose canonical name, when present, lives in the bytecode's `debug_info` ([`S-085`](docs/spec/behavior/sandbox.md), [`S-146`](docs/spec/behavior/sandbox.md)); structural failures surface as `Kobako::BytecodeError` during the first invocation's replay ([`S-148`](docs/spec/behavior/sandbox.md) / [`S-089`](docs/spec/behavior/sandbox.md)); the snippet table is sealed by the first invocation simultaneously with Service registration ([`S-087`](docs/spec/behavior/sandbox.md) / [`SV-031`](docs/spec/behavior/services.md)) | Host Gem | Runtime |
...
- Guest→host Handle restoration — the guest returns a Handle (bare and nested in an Array / Hash) as the `#eval` / `#run` result or as a yield-block result, and the host restores the original host object ([`T-054`](docs/spec/behavior/transport-dispatch.md), [`T-057`](docs/spec/behavior/transport-dispatch.md))
- Guest→host Handle arguments — the guest passes a Handle (bare and nested in an Array / Hash, positional or keyword) into a Service dispatch, and the host resolves each to its original host object before the call ([`T-051`](docs/spec/behavior/transport-dispatch.md), [`T-052`](docs/spec/behavior/transport-dispatch.md))
- Handle lifecycle over Sandbox teardown, and cross-invocation Handle invalidity — a Handle obtained in invocation N used as a target in invocation N+1 surfaces as `Kobako::ServiceError` with `type="undefined"` when not rescued within the guest ([`T-038`](docs/spec/behavior/transport-dispatch.md))
- gvl: scheduling parity and concurrent per-invocation isolation — a `gvl: :release` invocation produces the same outcome as `:hold` for the same program ([`RT-022`](docs/spec/behavior/runtime.md)), and Threads invoking concurrently — each on its own Sandbox, or sharing one and distinguishing invocations by their per-invocation `ctx.bind` identity — each mint and round-trip only their own Capability Handles with no cross-invocation misdelivery, over both `#eval` and `#run` ([`RT-004`](docs/spec/behavior/runtime.md), [`RT-002`](docs/spec/behavior/runtime.md)). A differential fuzz drives randomized Handle-minting dispatch trees under both modes and across parallel Threads, following the seed-and-coverage harness discipline of the Layer 1 codec fuzz
- Block / yield round-trip — a Service method receives a block via `&block` and yields one or more times; covers each Yield Reply tag (`0x01` ok, `0x02` break with [`T-089`](docs/spec/behavior/transport-yield.md) unwind semantics, `0x04` error from a block exception, and the unsupported-`return` path of [`T-134`](docs/spec/behavior/transport-yield.md) raising at the yield site), lambda-block `break` silent return ([`T-090`](docs/spec/behavior/transport-yield.md)), and nested dispatch frames ([`T-092`](docs/spec/behavior/transport-yield.md))
