...
- The `kobako` gem name, the public Ruby class names below, and the documented public methods on those classes are stable public contracts:
  ...
  | `Kobako::Extension` | The Extension contract, provided as a value object — `.new(name:, source:, backend:, depends_on:)` with `backend` and `depends_on` optional — together with the nested `Kobako::Extension::Backend` value object (`.new(path:, provider:)`). `Sandbox#install` duck-types on the four readers `name` / `source` / `backend` / `depends_on`, so a Host App or gem may pass any conforming object in place of the bundled value type |
...
| B-55..B-57 | Extension installation (`#install`) — composition, backend provider resolution, dependency presence | [`behavior/extension.md`](docs/behavior/extension.md) |
...
| **Extension::Backend** | The host attachment of an Extension — a value pairing `path` (the constant path the backend binds at, single-segment or nested, spelling the guest constant the idiom routes to) with `provider` (the source of the bound object). A provider that is not itself callable is the bound object, resolved once for the Sandbox's life; a callable provider is invoked once per invocation to yield that invocation's object, callability being the sole discriminator (B-56). Provider identity is resource identity: one provider shared by several Extensions resolves once per invocation to a single shared object. `Kobako::Extension::Backend` is the bundled value type — `#install` duck-types on `path` / `provider` — and in the Rust SDK the concept is `struct Backend` carrying a `Provider` enum (`Static` / `PerInvocation`), where the fixed-vs-fresh choice is explicit rather than inferred from callability. | Yes — nested under `Kobako::Extension` |
