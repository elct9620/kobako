...
- A `Kobako::Sandbox` may be invoked concurrently from multiple Ruby Threads, on distinct Sandboxes or a shared one; a single Thread runs at most one invocation at a time (see B-22). A host Service object bound once and shared across concurrently-invoking Threads must itself be thread-safe — a `provider:` or `ctx.bind` object is resolved per invocation and carries no such obligation.

**Output guarantees:**
- Every Sandbox invocation (`#eval` or `#run`) either returns a frozen `Kobako::Execution` or raises exactly one of `Kobako::TrapError`, `Kobako::SandboxError`, or `Kobako::ServiceError` — no other outcome is possible. The `Execution` carries that run's `#value` (`#eval`'s last-expression value or `#run`'s entrypoint return value) together with its output captures and usage; a raised error carries the same `Execution` on `#execution` (B-61).
...
- No capability state carries between invocations on the same Sandbox instance — successive or concurrent — regardless of verb; each invocation runs on its own per-invocation state (B-03, B-22)
- The `kobako` gem name, the public Ruby class names below, and the documented public methods on those classes are stable public contracts:
  ...
  |-----------------------|---------|
  | `Kobako::Sandbox` | `#bind` (with no object it declares a fillable, B-62), `#install`, `#preload`, `#eval`, `#run` (each accepting an optional `{ \|ctx\| ctx.bind(...) }` per-eval override block, B-63); configuration readers `#wasm_path` and `#options` (the `Kobako::SandboxOptions` value object), and the six option readers `#timeout` / `#memory_limit` / `#stdout_limit` / `#stderr_limit` / `#profile` (B-54) / `#gvl` (B-64) that forward to it |
...
- **Attribution is two-step:** Step 1 — if the Wasm engine reports a trap (including configured-cap traps), raise `Kobako::TrapError` or its named subclass (`Kobako::TimeoutError` per E-19, `Kobako::MemoryLimitError` per E-20). Step 2 — otherwise dispatch on the outcome envelope first-byte tag (`0x01` ok, `0x02` panic). Zero-length outcome bytes or unknown tags raise `Kobako::TrapError` as wire-violation fallback.
...
- **Extension installation composes preload and bind, adding no wire:** `#install` (B-55) registers an Extension's `source` as a preloaded snippet (B-32) and, when the Extension carries a `backend`, binds `backend.path` as a Service (B-08) — so the mechanism is entirely host-side setup over the two existing verbs and touches no wire, codec, or Guest Binary surface. The bound path enters Frame 1 at install time and is sealed with the other registries (B-33); the object behind it is chosen per invocation by which keyword the backend declares — a fixed `object:`, a per-invocation `provider:` resolved fresh each invocation, or a fillable defaulting to the `Kobako::Unresolved` sentinel (B-56 / B-62). An Extension's `depends_on` set is checked for presence at the seal (B-57). `source` is mandatory: a host object with no guest idiom is bound with `#bind` directly, not installed.
...
| B-07..B-11, B-62, B-63 | Service binding registration, including fillable paths defaulting to `Kobako::Unresolved` (B-62) and the per-eval `ctx.bind` override block (B-63) | [`behavior/registration.md`](docs/behavior/registration.md) |
| B-55..B-57 | Extension installation (`#install`) — composition, backend provider resolution (static / per-invocation / fillable), dependency presence | [`behavior/extension.md`](docs/behavior/extension.md) |
...
| B-22 | Concurrent-invocation isolation (distinct or shared Sandbox) | [`behavior/runtime.md`](docs/behavior/runtime.md) |
| B-64 | The `gvl:` scheduling mode — releasing Ruby's GVL for the guest span so distinct Sandboxes run host-parallel | [`behavior/runtime.md`](docs/behavior/runtime.md) |
...
| B-38 | Guest construction of a bound-Service proxy yields a capability-inert instance — forwarding rides on the constant at class level, so the instance carries no dispatch; construction is not a capability gate | [`behavior/security.md`](docs/behavior/security.md) |
| B-39 | Guest construction of a `Kobako::Handle` is blocked (`.new` / `.allocate` raise) so an exact instance arises only from the decoder and is always host-issued, keeping the receiver-identity check (B-59) sound; the host's per-invocation `Catalog::Handles` membership stays the capability gate (E-13) | [`behavior/security.md`](docs/behavior/security.md) |
...
| B-45 | The host's WASI-boundary denial of ambient wall-clock time and entropy under the default hermetic profile, making guest execution deterministic but for values a Service injects | [`behavior/security.md`](docs/behavior/security.md) |
...
| B-59 | Guest→host dispatch derives its target from the receiver's exact identity — an exact `Kobako::Handle` by its id, a class by its constant path — and refuses guest-side any other receiver (a Handle subclass, or a foreign object that mixed in the seam), so a guest cannot drive a Handle-targeted dispatch off fabricated state | [`behavior/security.md`](docs/behavior/security.md) |
| B-65 | No Handle id reaches an object the invocation did not deliver — an arbitrary integer presented as a Handle resolves to an object the guest already holds or to nothing, because the `Catalog::Handles` table is minted per invocation and discarded whole, carrying any un-delivered id out of existence with it | [`behavior/security.md`](docs/behavior/security.md) |
| B-60 | A guest-held `Kobako::Handle` is frozen at mint, so the guest cannot re-point or forge its id (reflective mutation raises `FrozenError`, `dup` / `clone` stay frozen); with blocked construction (B-39) this makes id unforgeability in-guest-enforced as defense-in-depth, the host's `Catalog::Handles` membership staying the gate | [`behavior/security.md`](docs/behavior/security.md) |
...
| B-52 | Guest-side JSON parse / generate as a guest-internal compute capability that projects to wire types when a result crosses the boundary | [`behavior/security.md`](docs/behavior/security.md) |
| B-53 | The JSON capability-reference boundary — `parse` cannot forge a Capability Handle, and `generate` refuses a Handle / a bound constant / un-opted object via an `Object`-rooted serialization hook rather than dispatching to the host | [`behavior/security.md`](docs/behavior/security.md) |
| B-54 | The isolation-profile ladder `permissive < hermetic` — `hermetic` names the B-45 ambient-denial posture with no filesystem / environment / network reachability beyond the wire ABI's single host import and the captured output; `permissive` grants live ambient time and entropy and nothing else — and the `Sandbox.new` `profile:` request the runtime builds and declares, failing construction when the declaration falls below the request | [`behavior/security.md`](docs/behavior/security.md) |
...
| `Kobako::SandboxError` | E-04..E-10, E-26..E-28, E-31, E-32, E-36..E-38, E-54, E-55 — E-37 / E-38 raised as the `Kobako::BytecodeError` subclass, E-27 as the `Kobako::UndefinedEntrypointError` subclass, E-10 and the dispatch-path half of E-07 as the `Kobako::Transport::Error` subclass |
| `Kobako::ServiceError` | E-11..E-13, E-15, E-21..E-23, E-43, E-44, E-48, E-57 — E-12 / E-13 / E-43 / E-48 raised as the `Kobako::NoServiceError` subclass, E-15 as the `Kobako::ServiceArgumentError` subclass |
...
|------|-----------|---------|
| **Sandbox** | The reusable configuration unit instantiated by `Kobako::Sandbox`. Owns one `Runtime` and the registries sealed at the first invocation — `Catalog::Services`, `Catalog::Snippets`, `Catalog::Extensions` — and holds no per-invocation state of its own. Exposes two invocation verbs — `#eval(code)` for one-shot mruby source execution (B-02 / B-03 / B-06) and `#run(target, *args, **kwargs)` for entrypoint dispatch into a preloaded constant (B-31), each driving one `Context` and returning a frozen `Kobako::Execution` (B-61) — plus the setup verbs `#bind` (B-08 / B-62), `#install` (B-55), and `#preload` accepting either `code:` plus `name:` for mruby source or `binary:` for RITE bytecode (B-32). Enforces three configurable per-invocation caps — wall-clock timeout, linear memory cap, and per-channel output cap — each independently disableable with `nil`, and carries a per-Sandbox `gvl:` scheduling mode (`:hold` default / `:release`) governing whether an invocation releases the GVL for host-parallel guest execution (B-64). Maps to the Ruby class `Kobako::Sandbox`. | Yes — `Kobako::Sandbox` is stable public API |
| **Context** | The per-invocation driver behind one `#eval` / `#run`. Owns that run's `Catalog::Handles`, its resolved per-invocation Extension backends, and its captures and usage, so no per-invocation state lives on the reusable Sandbox. Resolves each guest→host dispatch target by layering the per-eval `ctx.bind` overrides over this run's provider results over the sealed base bindings (B-63), then settles the run into its `Kobako::Execution`. Yielded to the optional `#eval` / `#run` override block and spent once that block returns. Maps to the Ruby class `Kobako::Context`. | Partially — `#bind` inside the override block is the stable public surface (B-63); every other member is internal |
| **Execution** | Returned on success and carried on a failed run's invocation-outcome error via `#execution` (B-61); its `#failed?` predicate is `true` iff the run failed, so a returned and a carried Execution stay distinct even when both `#value` are `nil`. Bundles that run's `#value` (the deserialized guest result), its `#stdout` / `#stderr` captures with truncation predicates (B-04), and its `#usage` (B-35). Maps to the Ruby class `Kobako::Execution`. | Yes — `Kobako::Execution` is stable public API |
...
| **Receiver** | The host object a dispatch resolves its Call `target` to — the object the Call's `method` runs on. A constant-path target resolves to the Service bound at that path; a Handle target resolves to the object its ID references in `Catalog::Handles`. Both target kinds resolve to a Receiver under the same dispatch constraints — the reflection floor (B-42) and the `respond_to_guest?` narrowing (B-50) bind bound-constant and Handle receivers identically; a Receiver whose predicate denies every name is opaque: the guest holds it, passes it, and returns it, but can call nothing on it. Names the role an ordinary host object occupies during a dispatch — no class of its own exists on either side. | No |
...
| **BytecodeError** | `Kobako::BytecodeError` | `Kobako::SandboxError` | `#preload(binary:)` bytecode failed structural validation during the first invocation's snippet replay — RITE version mismatch (E-37) or corrupt body (E-38). Backtrace attributes to the bytecode's `debug_info` filename when the bytecode carries one. |
| **UndefinedEntrypointError** | `Kobako::UndefinedEntrypointError` | `Kobako::SandboxError` | A `#run` target names no top-level constant (E-27). Carries `#name`, the entrypoint asked for, and `#available`, the top-level constants the preloaded snippets contributed — so the caller corrects the name from the error rather than by inspecting the guest source. |
| **NoServiceError** | `Kobako::NoServiceError` | `Kobako::ServiceError` | A dispatch reached no Service method: the bound path holds nothing, the Capability Handle is not live in this invocation, or the method is absent or outside the guest surface (E-12, E-13, E-43, E-48). The causes stay indistinguishable, so what a Host App learns is that this call will not succeed by being retried. |
| **ServiceArgumentError** | `Kobako::ServiceArgumentError` | `Kobako::ServiceError` | A dispatch reached the Service method but its arguments did not fit — an unknown keyword, or an arity mismatch (E-15). |
| **BlockError** | `Kobako::BlockError` | `Kobako::Error` | Raised at a Service method's `yield` site when the guest block it yielded to failed (B-24), carrying the guest-side class on `#klass`. It is not an invocation outcome and carries no `Execution`: a Service rescues it, or it returns to the guest, which continues the exception it raised. |
| **YieldValueError** | `Kobako::YieldValueError` | `Kobako::Error` | Raised at a Service method's `yield` site when a yield argument has no wire representation, so the round-trip never left the host and the block never ran (E-57). Like `BlockError` it is not an invocation outcome and carries no `Execution`: a Service rescues it, or the dispatch answers as that Service having failed. |
...
- **Capability Handle contract:** opaque on both sides of the boundary (cannot be dereferenced; the constructor is internal to the Host Gem on the host and blocked on the guest (B-39), so neither side turns a bare integer into a Handle, and the host's `Catalog::Handles` membership — not construction blocking — is the capability gate); host-allocated when a Service returns a non-wire-representable object (B-14, guest→host return path) or when `#run` receives non-wire-representable arguments (B-34, host→guest argument path), except reflective gadget objects (`Binding` / `Method` / `UnboundMethod`), which are never wrapped (B-43); scoped to a single invocation (each `#eval` / `#run` mints its own `Catalog::Handles`); ID bounded by `0x7fff_ffff` (allocation beyond raises `Kobako::HandleExhaustedError`, no silent wraparound).
- **Fault:** `type` (a reserved value naming which failure it reports — `"runtime"`, `"argument"`, `"undefined"`, `"internal"`, `"block"`, each stable across releases) and `message` (string), and nothing else: a Fault travels host→guest, so it carries only what its author can keep bounded — which is also why a guest's own block failure comes back as a category rather than as host-supplied detail (B-24). It is the whole of a Reply's fault body (`tag=1`) and rides the core envelope rather than the payload, so a guest reads a refusal without reaching any codec. The set is open and additive: a receiver reads a `type` it predates as `"undefined"` and skips a Fault field it predates, so an addition costs no ABI version increment while a change to what an existing value or field *means* does.
...
| N-6 | A concept has exactly one name; no synonyms appear in the same document or public surface. The names, and the names ruled out, are declared in [`.spec/glossary.json`](.spec/glossary.json), scoped to the files each governs; every use of a ruled-out name is reported against the line that uses it | All layers of this specification |
...
| Under the default `hermetic` profile the guest's WASI ambient sources are denied: `wasi:clocks` (wall and monotonic) is frozen and `wasi:random` is a constant stream, so no real time or host entropy reaches guest code; under `permissive` the same sources read the host's live clocks and entropy (B-45 / B-54). The per-invocation `timeout` is measured on the host clock and enforced by the engine on both rungs, independent of the guest's `wasi:clocks` | Runtime | Runtime |
...
| Single-Invocation Slot: Runtime holds at most one active Invocation per OS thread for the duration of any invocation (`#eval` or `#run`). Nested host→guest dispatch (Service calls Service via a yielded block which calls another Service) shares the same Invocation; nested dispatch frames stack within it (B-28). There is no stack of Invocations — at most one per thread. The slot licenses the guest-side per-thread statics (`MRB` slot, `BLOCK_STACK`, `OUTCOME_BUFFER`), the host-side per-invocation state (active caller pointer, deadline, wall-clock entry, memory peak, captures), and the per-thread GVL nesting state that governs release (B-64) | Runtime, Host Gem | Runtime |
| The `gvl:` mode is per-Sandbox and fixed at construction: under `:release` an invocation releases Ruby's GVL only for the span of guest execution and re-acquires it for each guest→host dispatch callback; `:hold`, the default, holds the GVL throughout. Releasing changes scheduling only — every isolation (B-03), Handle-lifecycle (B-18), capture (B-04), and outcome anchor holds identically, whether concurrent Threads use distinct Sandboxes or share one (B-64) | Runtime, Host Gem | Runtime |
| The wasmtime driver crate (`kobako-wasmtime`) declares no `magnus` dependency — only `ext/kobako` links `magnus`. The GVL-released span (B-64) calls into this driver, so the absent declaration is what keeps a Ruby VALUE out of the span's reach by construction; a build gate rejects the declaration in the driver's manifest | Architecture, Host Gem | Build-time |
...
- gvl: scheduling parity and concurrent per-invocation isolation — a `gvl: :release` invocation produces the same outcome as `:hold` for the same program (B-64), and Threads invoking concurrently — each on its own Sandbox, or sharing one and distinguishing invocations by their per-invocation `ctx.bind` identity — each mint and round-trip only their own Capability Handles with no cross-invocation misdelivery, over both `#eval` and `#run` (B-03 / B-22 / B-37). A differential fuzz drives randomized Handle-minting dispatch trees under both modes and across parallel Threads, following the seed-and-coverage harness discipline of the Layer 1 codec fuzz
- Block / yield round-trip — a Service method receives a block via `&block` and yields one or more times; covers each Yield Reply tag (`0x01` ok, `0x02` break with B-25 unwind semantics, `0x04` error from a block exception, and the unsupported-`return` path of E-21 raising at the yield site), lambda-block `break` silent return (B-27), and nested dispatch frames (B-28)
