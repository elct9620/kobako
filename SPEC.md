...
- The host↔guest message codec: a fixed-layout core envelope carrying an opaque payload, with MessagePack as the default payload codec and its three registered ext types (Symbol `0x00`, Capability Handle `0x01`, Fault `0x02`)
...
| `Kobako::SandboxError` | E-04..E-10, E-21..E-23, E-26..E-28, E-31, E-32, E-36..E-38, E-54, E-55 — E-37 / E-38 raised as the `Kobako::BytecodeError` subclass, E-27 as the `Kobako::UndefinedEntrypointError` subclass |
...
| **Call** | A message inviting the other side to run one method and answer. Carries `target`, `method`, the invocation payload, and `block_given`. Either side may issue one: the guest issues a Call to reach a bound Service or a Handle; the host issues a Call to re-enter a Block it received (the Yield Call, which carries only the yield arguments). Maps to the Ruby class `Kobako::Transport::Call` and to `Call` in each core-envelope peer (`kobako_codec::envelope`, `kobako_runtime::envelope`). | No |
| **Reply** | The answer to one Call, produced by whichever side received it. A dispatch Reply is a Success (`tag=0`, value) or Fault (`tag=1`, fault body) variant; a Yield Reply additionally carries the `break` outcome a Block can produce. One Reply per Call, always — there is no partial or streaming answer. Maps to `Reply` / `YieldReply` in each core-envelope peer (`kobako_codec::envelope`, `kobako_runtime::envelope`), and has no Ruby counterpart — the native side frames every Reply, so the Host Gem's Ruby layer answers a Call with the arm it selected and that arm's payload bytes rather than with a value object. | No |
...
| **PayloadCodec** | The obligations a payload codec fills — the payload positions of a Call, a Run, a Reply's two arms, a Yield Reply, and an Outcome. A guest names its choice on `MrbGuest::Codec`; a Rust host reads the payload itself in `Receiver::call`. The Ruby frontend has no such seam and speaks the default codec. Maps to `kobako_mruby::PayloadCodec`. | Yes — third parties implement it (N-9) |
...
**Wire-level error class:** The string `"Kobako::Transport::Error"` appears as the `class` field value in a Panic envelope (defined in [`docs/wire-contract.md`](docs/wire-contract.md) § Outcome Envelope; the governing summary lives below in `### Wire Contract`) to signal that the Transport layer detected a wire violation. On the host side this maps to a raised `Kobako::Transport::Error`, a `Kobako::SandboxError` subclass — so a single `rescue Kobako::SandboxError` still catches it, while callers that want to single out wire-violation paths from ordinary script failures can `rescue Kobako::Transport::Error` directly. (The guest mruby class `Kobako::Transport::Error` exists only to be raised inside the Guest Binary; it is captured by the guest's top-level handler and converted into the panic envelope string.)
...
- **Fault:** `type` (one of three reserved values — `"runtime"`, `"argument"`, `"undefined"` — stable across releases) and `message` (string), and nothing else: a Fault travels host→guest, so it carries only what its author can keep bounded. Its sole legal wire position is a Reply's fault body (`tag=1`), and everything inside that body is itself a payload position — so a Fault never nests inside a Fault, and a guest→host payload carrying one anywhere is a wire violation the host rejects (E-50). Adding a new `type` requires a coupled host+guest release.
- **Outcome envelope:** per-invocation final result, distinct from a per-dispatch Reply; two variants — Result envelope (serialized last mruby expression) or Panic envelope (`origin` / `class` / `message` / `backtrace` / `available`, where `origin="service"` attributes to `Kobako::ServiceError` and every other value attributes to `Kobako::SandboxError`; `available` lists the names the invocation could have used in place of the one it named, empty when the failure offers no correction). Every Panic field is typed at the core envelope, so a Panic carries no codec-encoded content and attribution never depends on one. An outcome the core envelope cannot frame, an absent one included, raises `Kobako::TrapError`.
...
- **A replacement codec's obligations** are the positions it must fill, not the encoding it uses: the Call and Run payloads must express positional and keyword arguments distinguishably; a Reply's fault body must carry the three reserved `type` values plus a message; and Handles ride payload positions only if the codec has a representation for them — a Handle `target` is an envelope field and stays reachable regardless. The obligations, and the seam each endpoint names its choice at, are specified in [`docs/customization.md`](docs/customization.md).
...
- **Envelope framing:**
  ...
  | Reply | `tag` u8 (`0` ok / `1` fault) · body remainder — the value or the fault |
  | Outcome | `tag` u8 (`0x01` result / `0x02` panic) · body remainder — the value, or a Panic |
  | Result | the payload alone (the Outcome tag discriminates) |
  ...
  | Error Record | `class` · `message` byte strings · `backtrace` byte-string list |
...
- **Payload type set (MessagePack codec):** exactly 12 entries — `nil`, `bool`, `int`, `float`, `str`, `bin`, `array`, `map`, and the three ext codes `0x00` (Symbol), `0x01` (Capability Handle), `0x02` (Fault). Any msgpack type or ext code outside this set is a wire violation; both sides reject without attempting to decode. Ext code assignments are stable for the life of an ABI version: `0x00` Symbol (kwargs map keys MUST be this type), `0x01` Capability Handle (`fixext 4`, big-endian u32 ID, cap `0x7fff_ffff`), `0x02` Fault (embedded msgpack map carrying `type` / `message`, legal only as the whole of a Reply's fault body).
...
- **Single-dispatch size cap:** 16 MiB in either direction, applied to the whole envelope rather than the payload alone. Exceeding the cap is a wire violation; the Host Gem walks the trap path.
...
- **Consistency guarantee:** each layer is verified against its own second implementation, and neither layer has a single implementation whose output is its own definition of correct. The core envelope is pinned by a byte oracle between `crates/kobako-runtime` and `crates/kobako-codec` plus golden vectors on both sides; the payload codec by bidirectional round-trip fuzz between the Ruby host and the Rust/mruby guest, covering all 12 wire types and all 3 ext types in both directions. The core layer's cross-check is cross-implementation rather than cross-language — a deliberate trade, taken because the type-mapping ambiguity two languages disagree about stays entirely in the payload codec. Any failure at either layer is a wire regression that blocks release. The harness contract is specified in Implementation Standards § Testing Style.
...
| N-9 | Every name a third party implements against carries a declared grade — stable, append-only, or replaceable — and the declaration lives in [`docs/customization.md`](docs/customization.md). A grade is a commitment: a stable name does not change without an ABI version increment, and a replaceable interface states the obligations an implementation is held to | The Rust host SDK, the guest assembly points (`Guest`, `MrbGuest`, `export_guest!`, `PayloadCodec`), and the engine contract |
...
- **`crates/`** — the publishable Rust crates a non-Ruby embedder builds on: `kobako-codec` (the portable wire tier — codec, envelopes, outcome — shared by both sides of the wasm boundary), `kobako-runtime` (the engine-neutral runtime contract), and `kobako-wasmtime` (the wasmtime driver — owns the Wasm engine lifecycle and implements the host-side import function `__kobako_dispatch`). The ext consumes `kobako-runtime` and `kobako-wasmtime` as path dependencies; the guest crates consume `kobako-codec` the same way across the workspace boundary.
...
The boundary rule is: **`ext/` is private to the Host Gem and must never be imported by downstream gems**; `lib/` is the stable public surface. Three Cargo workspaces keep the dependency graphs separate: the root workspace (`ext/kobako` only), `crates/` (the publishable crates), and `wasm/` (the guest). The root `Cargo.toml` excludes `wasm/`, `vendor/`, and `crates/` — the ext and the guest workspace consume `crates/` members as plain path dependencies (`kobako-codec` is engine-free, so its presence in the guest graph pulls in no host-only code), and the isolation prevents host-only crates (e.g., `wasmtime`) from appearing in the wasm32 dependency graph.
...
| `Sandbox#eval`'s `Execution#value` is the last mruby expression and `Sandbox#run`'s is the entrypoint's `#call` value, both carried via the Result envelope path. A value the guest hands across the boundary that has no wire representation is rejected rather than coerced — no path substitutes an implicit `inspect` / `to_h` / `to_s` string: an invocation return takes the Panic envelope path (E-06), a yield-block result fails the yield round-trip (E-22), and a dispatch argument or kwargs value fails at the dispatch call site (E-55) | Guest Binary, Wire Spec | Test-time |
...
- Wire-violation edge cases — `len=0`, unknown tag, Result envelope with unrepresentable value
...
- `crates/` — host-side Rust crates (`kobako-runtime`, `kobako-wasmtime`) the ext consumes as path dependencies; the two crates ship in the published gem, the `crates/` workspace manifest and lock do not
