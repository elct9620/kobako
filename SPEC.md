...
| **Reply** | The answer to one Call, produced by whichever side received it. A dispatch Reply is a Success (`tag=0`, value) or Fault (`tag=1`, fault envelope) variant; a Yield Reply additionally carries the `break` outcome a Block can produce. One Reply per Call, always — there is no partial or streaming answer. Maps to `Reply` / `YieldReply` in each core-envelope peer (`kobako_codec::envelope`, `kobako_runtime::envelope`), and has no Ruby counterpart — the native side frames every Reply, so the Host Gem's Ruby layer answers a Call with the arm it selected and that arm's payload bytes rather than with a value object. | No |
...
- **Fault envelope:** `type` (one of three reserved values — `"runtime"`, `"argument"`, `"undefined"` — stable across releases), `message` (string), `details` (optional structured data carrying caller-actionable diagnostics). Its sole legal wire position is a Reply's fault body (`tag=1`); a guest→host payload carrying one anywhere is a wire violation the host rejects (E-50). Adding a new `type` requires a coupled host+guest release.
