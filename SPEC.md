...
| B-38 | The guest's inability to construct a bound-Service proxy (`MyService::KV.new` / `.allocate` raise `NoMethodError`, attributed via E-04) | [`behavior/security.md`](docs/behavior/security.md) |
...
| **Proxy** | The guest capability module `Kobako::Proxy`. Guest code gains host-forwarding by mixing it in rather than inheriting a base class: a bound Service's constant `extend`s it — class-level calls forward as a Transport Request whose `target` is the constant's path — and `Kobako::Handle` `include`s it, so instance-level calls forward with the instance's Handle ID as `target`. The module supplies the shared `method_missing` / `respond_to_missing?` dispatch seam and the construction block (`new` / `allocate` raise), holds no per-instance state, and has no host-side counterpart — the host represents a bound Service as a path entry in `Catalog::Services`, not as a class. | No |
