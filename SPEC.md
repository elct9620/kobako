...
| **Service** | A Host Ruby object bound into the sandbox at a constant-path name (`MyService::KV`, or a top-level `File`). The only mechanism by which guest code can access host resources. | In scope |
| **Wire Spec** | The two-layer contract governing all host↔guest Transport messages: a fixed-layout core envelope carrying routing and outcome attribution, and an opaque payload a replaceable codec owns. Not a runtime object — it is a protocol that both Host Gem and Guest Binary implement independently, each layer against its own second implementation. | In scope |
...
| **Execution** | The frozen result of one `#eval` / `#run`, returned on success and carried on a failed run's invocation-outcome error via `#execution` (B-61); its `#failed?` predicate is `true` iff the run failed, so a returned and a carried Execution stay distinct even when both `#value` are `nil`. Bundles that run's `#value` (the deserialized guest result), its `#stdout` / `#stderr` captures with truncation predicates (B-04), and its `#usage` (B-35); frozen at return, one per invocation and never mutated by a later one. Names the record a call produces — distinct from the in-flight call the Single-Invocation Slot invariant governs and from the host→guest Run envelope, which name the call itself. Maps to the Ruby class `Kobako::Execution`. | Yes — `Kobako::Execution` is stable public API |
