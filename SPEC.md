...
- **A replacement codec's obligations** are the positions it must fill, not the encoding it uses: the Call and Run payloads must express positional and keyword arguments distinguishably; and Handles ride payload positions only if the codec has a representation for them — a Handle `target` is an envelope field and stays reachable regardless. A Reply's fault body is not among them: a Fault is kobako's own data and rides the envelope, so a replacement schema neither encodes nor reads one. The obligations, and the seam each endpoint names its choice at, are specified in [`docs/customization.md`](docs/customization.md).
