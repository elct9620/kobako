...
| **Call** | A message inviting the other side to run one method and answer. Carries `target`, `method`, the invocation payload, and `block_given`. Either side may issue one: the guest issues a Call to reach a bound Service or a Handle; the host issues a Call to re-enter a Block it received (the Yield Call, which carries only the yield arguments). Maps to the Ruby class `Kobako::Transport::Call` and the Rust type `kobako_codec::transport::Call`. | No |
| **Reply** | The answer to one Call, produced by whichever side received it. A dispatch Reply is a Success (`status=0`, value) or Fault (`status=1`, fault envelope) variant; a Yield Reply additionally carries the `break` outcome a Block can produce. One Reply per Call, always — there is no partial or streaming answer. Maps to the Ruby classes `Kobako::Transport::Reply` / `Kobako::Transport::YieldReply` and the Rust types `kobako_codec::transport::Reply` / `YieldReply`. | No |
