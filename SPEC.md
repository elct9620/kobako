...
- The host↔guest message codec: a fixed-layout core envelope carrying an opaque payload, with MessagePack as the default payload adapter and its three registered ext types (Symbol `0x00`, Capability Handle `0x01`, Fault `0x02`)
...
| **Service** | A Host Ruby object bound into the sandbox at a constant-path name (`MyService::KV`, or a top-level `File`). The only mechanism by which guest code can access host resources. | In scope |
| **Wire Spec** | The two-layer contract governing all host↔guest Transport messages: a fixed-layout core envelope carrying routing and outcome attribution, and an opaque payload a replaceable adapter owns. Not a runtime object — it is a protocol that both Host Gem and Guest Binary implement independently, each layer against its own second implementation. | In scope |
...
| **Capability Handle** | A Handle that represents a Ruby object outside the payload adapter's type mapping — either a stateful resource (session, connection, any object the adapter cannot transmit by value) returned from a Service call, or a non-wire-representable value (`StringIO`, a custom Env / Context object) supplied as a `#run` argument. Rides the core envelope's `target` field as a bare ID, and a payload position as the adapter's Handle representation (MessagePack ext type `0x01`). "Capability Handle" is used when emphasizing the capability-granting semantics; "Handle" is used for brevity elsewhere — both refer to the same concept. | No — same visibility as Handle; no distinct class exists |
...
The abstract logical shape of every host↔guest message exchanged during a `#run` invocation is specified in detail in [`docs/wire-contract.md`](docs/wire-contract.md). It is a Consistency-layer contract: both sides implement it independently, and a kobako gem release ships exactly one version of it. Byte-level encoding is specified across the two layers [`docs/wire-codec.md`](docs/wire-codec.md) anchors — the fixed-layout core envelope and the payload adapter. The decisions below govern the contract; the linked documents reproduce them as field tables, envelope shapes, and per-position rules.
...
- **Two encoding layers:** the **core envelope** is a fixed layout — length-prefixed byte strings, `u8` tags, big-endian `u32` — carrying only routing and outcome attribution, and it is not MessagePack. The **payload** it hands through is an opaque byte string a payload adapter owns. Byte-level references: [`docs/wire/envelope.md`](docs/wire/envelope.md) and [`docs/wire/payload-msgpack.md`](docs/wire/payload-msgpack.md).
- **MessagePack is the default payload adapter, not the wire's only codec.** The bundled Guest Binary and the Ruby frontend speak it, and a host that chooses nothing gets it. Two endpoints that agree on another schema replace it and carry no MessagePack dependency; the core envelope and the ABI surface are unchanged by that substitution, so it is not an ABI version increment.
- **The two decodes are separable.** Decoding an envelope requires nothing from the adapter, and decoding a payload requires nothing from the envelope beyond its bytes and length. A frontend may therefore decode the two on different sides of its own internal boundaries, and an endpoint that only routes messages needs no adapter at all.
- **A replacement adapter's obligations** are the positions it must fill, not the encoding it uses: the Call and Run payloads must express positional and keyword arguments distinguishably; a Reply's fault body must carry the three reserved `type` values plus a message; and Handles ride payload positions only if the adapter has a representation for them — a Handle `target` is an envelope field and stays reachable regardless.
- **Envelope framing:**
  ...
  | Panic | `origin` · `class` · `message` byte strings · `backtrace` · `available` byte-string lists |
...
- **Payload type set (MessagePack adapter):** exactly 12 entries — `nil`, `bool`, `int`, `float`, `str`, `bin`, `array`, `map`, and the three ext codes `0x00` (Symbol), `0x01` (Capability Handle), `0x02` (Fault). Any msgpack type or ext code outside this set is a wire violation; both sides reject without attempting to decode. Ext code assignments are stable for the life of an ABI version: `0x00` Symbol (kwargs map keys MUST be this type), `0x01` Capability Handle (`fixext 4`, big-endian u32 ID, cap `0x7fff_ffff`), `0x02` Fault (embedded msgpack map carrying `type` / `message`, legal only as the whole of a Reply's fault body).
- **Call and Run payloads are a 2-element array** — `args` then `kwargs`, both always present. The positional-versus-keyword partition is an adapter concern: a schema without Ruby's call semantics carries its own shape and the envelope is unchanged.
...
- **Nesting depth is a per-document budget:** the core envelope does not nest — every field is a scalar, a byte string, or a flat list — so the 128-level bound belongs to the payload adapter, and the envelope and each payload it carries are separate documents with separate budgets.
- **Consistency guarantee:** each layer is verified against its own second implementation, and neither layer has a single implementation whose output is its own definition of correct. The core envelope is pinned by a byte oracle between `crates/kobako-runtime` and `crates/kobako-codec` plus golden vectors on both sides; the payload adapter by bidirectional round-trip fuzz between the Ruby host and the Rust/mruby guest, covering all 12 wire types and all 3 ext types in both directions. The core layer's cross-check is cross-implementation rather than cross-language — a deliberate trade, taken because the type-mapping ambiguity two languages disagree about stays entirely in the payload adapter. Any failure at either layer is a wire regression that blocks release. The harness contract is specified in Implementation Standards § Testing Style.
