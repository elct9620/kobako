...
- **Fault:** `type` (one of three reserved values — `"runtime"`, `"argument"`, `"undefined"` — stable across releases) and `message` (string), and nothing else: a Fault travels host→guest, so it carries only what its author can keep bounded. It is the whole of a Reply's fault body (`tag=1`) and rides the core envelope rather than the payload — every byte of one is kobako's, so a guest reads a refusal without reaching any codec, and the closed `type` set rides as a tag no endpoint can receive an unknown value of. Adding a new `type` requires a coupled host+guest release and an ABI version increment.
...
- **Payload type set (MessagePack codec):** exactly 11 entries — `nil`, `bool`, `int`, `float`, `str`, `bin`, `array`, `map`, and the two ext codes `0x00` (Symbol), `0x01` (Capability Handle). Any msgpack type or ext code outside this set is a wire violation; both sides reject without attempting to decode. Ext code assignments are stable for the life of an ABI version: `0x00` Symbol (kwargs map keys MUST be this type), `0x01` Capability Handle (`fixext 4`, big-endian u32 ID, cap `0x7fff_ffff`).
