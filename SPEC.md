...
| **HandleTable** | The host-side mapping from Handle IDs to Ruby objects. Owned by the Server. Reset at the start of every invocation (`#eval` or `#run`) and fully discarded when the owning Sandbox is collected. Not exposed to the Host App. | No |
