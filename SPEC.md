...
- **Capability Handle contract:** opaque on both sides of the boundary (cannot be dereferenced, and the constructor is internal to the Host Gem — neither guest mruby code nor Host App code can fabricate a Handle from a bare integer); host-allocated when a Service returns a non-wire-representable object (B-14, guest→host return path) or when `#run` receives non-wire-representable arguments (B-34, host→guest argument path), except reflective gadget objects (`Binding` / `Method` / `UnboundMethod`), which are never wrapped (B-43); scoped to a single invocation (`Catalog::Handles` reset at every `#eval` or `#run`); ID bounded by `0x7fff_ffff` (allocation beyond raises `Kobako::HandlerExhaustedError`, no silent wraparound).
...
| `Catalog::Handles` IDs are bounded by `0x7fff_ffff` (2³¹ − 1); exceeding the cap raises `Kobako::HandlerExhaustedError` immediately — no silent wraparound or truncation | Host Gem, wire layer | Runtime |
