...
| N-6 | A concept has exactly one name; no synonyms appear in the same document or public surface. The names, and the names ruled out, are declared in [`docs/spec/glossary.md`](docs/spec/glossary.md); the `rake gate:spec:glossary` gate holds a ruled-out name to zero uses across the corpus, so a name can only be ruled out once nothing says it | All layers of this specification |
