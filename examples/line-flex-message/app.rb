...
# receiver-less line-message-builder Flex DSL (`bubble do ... end`); the host
...
# simply that a real gem's DSL works unchanged inside that boundary.
#
# Usage:
...
module LineFlex
  ...
  # `method_missing` forwards a dynamic method name, with its positional and
  # keyword arguments, onto the wrapped Handle; when a block is given it
  # descends into the returned child node via `instance_eval` so the guest
  # writes a receiver-less builder. A block-less call returns the raw result.
  BUILD_SOURCE = <<~MRUBY
    class Build
  ...
  # The backend bound at the guest's `Flex`: it mints fresh root builders. A
  # bubble or carousel is a stateful node, so it crosses to the guest as a
  # Handle the guest descends into.
  class Studio
  ...
  def self.extension
    Kobako::Extension.new(
      name: :LineFlex,
      source: BUILD_SOURCE,
      backend: Kobako::Extension::Backend.new(path: "Flex", provider: -> { Studio.new })
    )
  end
