...
## B-37 — Guest returns a Capability Handle as the invocation result (Host-side Handle restoration)

This behavior refines the return semantics of B-06 (`#eval`) and B-31 (`#run`) for the case where the returned value is, or contains, a Capability Handle.

| Field | Value |
|-------|-------|
| **Initial State** | An invocation (`#eval` or `#run`) is completing without a `Kobako::TrapError`. The guest's result — the last mruby expression of `#eval`, or the entrypoint's `#call` return for `#run` — is a `Kobako::Handle` proxy the guest received earlier in this same invocation (from a Service return per B-14, or from a `#run` argument auto-wrap per B-34), or an Array / Hash containing one or more such proxies. |
| **Operation** | The guest serializes its result through the same wire codec used for every return value; each `Kobako::Handle` proxy encodes as an `ext 0x01` Capability Handle carrying its Handle ID, which the Result envelope `value` position accepts. On the host, after decoding the Outcome Result, the Host Gem walks the decoded value — descending into Array elements and Hash keys and values one level at a time — and replaces each decoded Capability Handle with the host-side object bound to its ID in this invocation's Catalog::Handles. Wire-representable leaves pass through unchanged. |
| **Result / Final State** | `#eval` / `#run` returns the original host-side object instance — the same object Catalog::Handles holds — at each position where the guest returned a Handle. A bare returned Handle yields that object directly; a nested return yields the Array / Hash with each Handle position replaced by its original object and all other structure preserved. The Host App receives ordinary host objects and never observes a `Kobako::Handle`. Restoration is a read against Catalog::Handles, not an allocation: no new Handle ID is issued and the table is unchanged. |
| **Notes** | B-37 is the symmetric inverse of B-34: B-34 wraps non-wire-representable objects into Handles on the host→guest argument path; B-37 restores Handles to their objects on the guest→host return path. Both operate on the same Catalog::Handles table under the same per-invocation lifecycle (B-15, B-19), and both walk only Array and Hash containers — neither descends into other object internals. Restoration resolves against the table as it stands at the end of the producing invocation, before the next invocation's reset (B-19); because the guest cannot fabricate a Handle (B-20), every Handle the guest can legitimately return resolves to a live object. A returned Handle ID with no live binding indicates a corrupted runtime and is raised as `Kobako::SandboxError` through the same wire-violation fallback as a malformed Result value (E-09). |
