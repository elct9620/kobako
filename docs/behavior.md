...
| **Notes** | B-37 is the symmetric inverse of B-34: B-34 wraps non-wire-representable objects into Handles on the host→guest argument path; B-37 restores Handles to their objects on each guest→host value path **whose value host code consumes**. Both operate on the same Catalog::Handles table under the same per-invocation lifecycle (B-15, B-19), and both walk only Array and Hash containers — neither descends into other object internals. Restoration resolves against the table as it stands while the producing invocation is live, before the next invocation's reset (B-19); because the guest cannot fabricate a Handle (B-20), every Handle the guest can legitimately return resolves to a live object. A returned Handle ID with no live binding indicates a corrupted runtime and is raised as `Kobako::SandboxError` through the same wire-violation fallback as a malformed Result value (E-09). Two yield paths are deliberately excluded: a `0x02` break value (B-25) returns to the guest Member call rather than to host code, so its Handle passes through unrestored to ride back on the same ID; and the `0x04` error YieldResponse (E-22) carries no value to restore. |
...
| **Notes** | This is the guest-side enforcement of B-20's rule that guest code has no path to fabricate a Handle; the Host App side is enforced separately at host pre-flight (E-29). `new` and `allocate` are the two construction entries mruby exposes on a class; both raise, registered to fire regardless of passed arguments rather than tripping an arity check first. The wire decoder's restoration path (B-14 / B-34) is unaffected: it constructs a Handle through an internal instance-allocation path that does not dispatch the Ruby `new` / `allocate` entries this behavior blocks. Parallels B-38 (the same block on the `Kobako::Member` proxy). |
...
| Field | Value |
|-------|-------|
| **Initial State** | A Sandbox executing mruby guest code. The Guest Binary provides `Regexp` and `MatchData` as guest-visible Ruby classes — a regexp literal (`/.../`), `Regexp.new` / `Regexp.compile`, and the `String` integration methods (`=~`, `match`, `match?`, `scan`, `gsub`, `sub`, `split`, `[]` / `slice`, `index`) compile and run patterns against `String` values. Neither class is among the 12 wire types (→ [`docs/wire-codec.md`](wire-codec.md) § Type Mapping). |
| **Operation** | Guest code compiles a pattern and matches it against a `String`, then uses the result inside the invocation — a `MatchData`, an Integer match index, `nil` for no match, captured substrings (positional or by name), or the match backref globals (`$~`, `$1`..`$9`, `$&`, `` $` ``, `$'`) set after each match. |
...
| **Notes** | Regexp is the "pure-compute third-party" capability named in the Scope layer's stdlib statement; like every guest stdlib capability it carries no per-feature wire contract — the closed 12-entry wire type set already excludes `Regexp` / `MatchData`, so projecting a result to wire types is structural, not a new envelope. Coverage tracks the curated regexp engine's surface, not the full CRuby `Regexp` / `MatchData` API: there are no `Encoding` objects, and match offsets and substring slices (`#begin` / `#end` / `#offset`, `pre_match` / `post_match`, `captures`) are byte-based. Which engine backs the capability — an allowlisted mruby mrbgem or a Rust capability gem composed into the Guest Binary shell — is an implementation choice below this contract, the same latitude the IO capability (B-04) takes between an mrbgem and a Rust gem. |
