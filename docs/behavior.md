...
| **Result / Final State** | A `Kobako::Catalog::Binding::Namespace` instance is created and associated with this Sandbox under the name `Name`. The namespace has no members yet. The method returns the new `Kobako::Catalog::Binding::Namespace` instance. The Sandbox's `Catalog::Binding` now tracks one additional namespace entry. |
...
| Field | Value |
|-------|-------|
| **Initial State** | A `Kobako::Catalog::Binding::Namespace` instance (returned by `sandbox.define`) with no member bound under the name `MemberName`. |
...
| **Result / Final State** | `object` is registered as the Member named `MemberName` within the namespace. Guest code can now reach this object via the two-level path `Name::MemberName`. The method returns the `Kobako::Catalog::Binding::Namespace` instance (`self`) to allow chaining. |
...
| **Result / Final State** | A new, independent `Kobako::Catalog::Binding::Namespace` is created alongside the existing namespaces. Each namespace's members are accessible to guest code only via that namespace's own path (e.g., `NamespaceA::Member` and `NamespaceB::Member` are distinct paths with no cross-namespace visibility). Namespaces on different Sandbox instances are fully isolated from each other. |
...
| **Result / Final State** | No new namespace is created. `sandbox.define(:Name)` returns the identical `Kobako::Catalog::Binding::Namespace` object previously created — the same object identity (Ruby `equal?`), not a new instance wrapping the same `Catalog::Binding` entry. All previously bound members remain in place. The Sandbox's `Catalog::Binding` is not modified. |
...
| Field | Value |
|-------|-------|
| **Initial State** | A `Kobako::Catalog::Binding::Namespace` instance with a member already bound under the name `MemberName`. |
...
| **Notes** | Each dispatch invokes the bound object's method exactly once. Keyword argument names travel on the wire as Symbols (→ [`docs/wire-codec.md`](wire-codec.md) § Type Mapping); the host passes them to `public_send` without further conversion. If the target path is not found in `Catalog::Binding`, a `ServiceError` is returned to the guest (covered in the Error Scenarios subsection). |
...
| **Result / Final State** | The guest receives the return value as the synchronous result of the method call, deserialized to the corresponding mruby type. The value is indistinguishable from a locally computed mruby value. No entry is added to the Catalog::Handler. |
...
| **Operation** | A return value is routed through the Handle allocation path if and only if its type is not wire-representable per the definition in B-13. The wire layer then automatically registers the object in the Sandbox's Catalog::Handler. |
| **Result / Final State** | The host-side object is stored in `Catalog::Handler` under a new opaque u32 Handle ID. The guest receives a Capability Handle (an opaque integer token) as the dispatch response value, not the object itself. The guest can pass this Handle as the `target` in subsequent dispatch requests to invoke methods on the same host-side object. The Host App has no API to create or inspect Handles directly — Handle allocation is an internal wire-layer operation. |
| **Notes** | Handle lifecycle (per-invocation scope, ABA protection, ID limits) is specified in the Handle lifecycle behaviors (B-15–B-21). The guest cannot dereference a Handle to a Ruby value; it can only use it as a target in further dispatch requests. The host→guest symmetric direction — `#run` arguments containing non-wire-representable objects — is governed by B-34, which routes through the same `Catalog::Handler` allocator and lifecycle rules. |
...
| Field | Value |
|-------|-------|
| **Initial State** | An invocation (`#eval` or `#run`) has just begun. The Catalog::Handler counter is reset to 1. No entries exist in the table. |
...
| Field | Value |
|-------|-------|
| **Initial State** | An invocation is in progress. The guest holds a `Kobako::Handle` (mruby object) obtained from a prior dispatch response (B-14) or from a `#run` argument auto-wrapped by the host (B-34) in the same invocation. The Handle's internal ID resolves to a live entry in `Catalog::Handler`. |
...
| **Result / Final State** | The Host Gem resolves `handle_a`'s ID against `Catalog::Handler` and invokes `public_send(:find, id)` on the host-side Ruby object that `handle_a` represents. If that call returns another stateful object, a new Handle `handle_b` is allocated and returned to the guest. Each step in the chain is an independent, synchronous dispatch; there is no implicit multi-hop traversal within a single wire call. |
| **Notes** | Chain depth is unbounded within a single invocation as long as each step produces a Handle that survives to the next call. Each intermediate Handle is a first-class entry in the Catalog::Handler and follows the same lifecycle rules as any other Handle. Every host object reachable by the guest must have entered through an explicit allocation path — either Service-returned (B-14) or `#run` argument auto-wrapped (B-34); there is no implicit intermediate binding path. |
...
| Field | Value |
|-------|-------|
| **Initial State** | A `Kobako::Sandbox` instance exists with zero or more completed invocations. The Catalog::Handler is owned by this Sandbox instance. |
| **Operation** | The Sandbox instance is garbage-collected or goes out of scope; Ruby reclaims it. |
| **Result / Final State** | The Catalog::Handler and all its entries are destroyed as part of Sandbox teardown. Every Handle that was issued during any invocation on this Sandbox is permanently invalid. No Handle entry is shared with, transferred to, or reachable from any other Sandbox instance. |
| **Notes** | Handles are not reference-counted and there is no explicit `release` API for individual entries. Validity is scoped to the owning Sandbox and the specific invocation in which the Handle was issued (B-18). A Handle that was valid in a prior invocation on this Sandbox is already invalid by the time the Sandbox is collected (per B-18); Sandbox teardown simply removes the ownership root. This applies uniformly to all Catalog::Handler entries regardless of allocation source — Service-returned (B-14) or host-injected via `#run` (B-34). |
...
## B-21 — Catalog::Handler exhaustion: allocation attempt beyond the ID cap

| Field | Value |
|-------|-------|
| **Initial State** | An invocation is in progress. The Catalog::Handler counter has reached `0x7fff_ffff` (2³¹ − 1), the maximum valid Handle ID. |
| **Operation** | The Host Gem's wire layer attempts to allocate one additional Handle for a new stateful return value. |
| **Result / Final State** | The allocation fails immediately with a `Kobako::SandboxError`. The counter is not incremented, no new entry is written to the Catalog::Handler, and no ID is silently truncated or wrapped. The error is raised to the Host App; the current invocation terminates. |
...
| **Notes** | `wall_time` is measured on the host side around the guest export call (`__kobako_eval` / `__kobako_run`); the bracket starts when the per-invocation caps are armed and ends when wasmtime returns control to the host, so the measurement includes time spent in host Service callbacks invoked during dispatch (consistent with the `timeout` accounting in B-01 Notes) but excludes everything that runs after the guest export returns — the post-export `OUTCOME_BUFFER` fetch (the `__kobako_take_outcome` call) and its msgpack decode, plus the stdout / stderr capture readout. `memory_peak` shares the same baseline accounting as `memory_limit` (B-01, E-20): the mruby image's initial allocation and any high-water mark left by prior invocations on the same Sandbox sit outside the measurement, so `memory_peak` reports how much linear memory the current user script alone caused `memory.grow` to allocate. Both readers are populated on every invocation outcome — value return, `Kobako::TrapError` (including `TimeoutError` / `MemoryLimitError`), `Kobako::SandboxError`, and `Kobako::ServiceError` — so the Host App can read `#usage` after rescuing a trap to diagnose how much of the budget the failing invocation consumed. On `MemoryLimitError`, `memory_peak` reports the largest delta that the limiter accepted before the trap rather than the rejected `desired` value, so `memory_peak` never exceeds `memory_limit`. `#usage` does not measure cross-invocation aggregates and does not feed billing; SPEC.md Non-Goals govern that boundary. |
...
| E-14 | The dispatch `target` Handle ID resolves to the `:disconnected` sentinel in `Catalog::Handler`; error `type="disconnected"` | B-16 — Handle referencing |
...
Raised by `Kobako::Sandbox.new` when the wasm runtime cannot be constructed from the configured `wasm_path` (B-01) — before any Guest Binary invocation runs. Construction is a setup verb, not an invocation: `SetupError` is therefore not one of the four invocation outcomes and does not pass through the two-step attribution decision, mirroring the E-16..E-18 setup-time treatment. Because no Sandbox instance is produced, the `TrapError` "discard and recreate" recovery contract does not apply — a `SetupError` reflects a deterministic artifact or environment fault, and retrying `Sandbox.new` against the same `wasm_path` fails identically until the underlying cause is fixed.
...
E-39 is a Host App programming error detected before any engine work; it raises `ArgumentError` and does not engage the attribution pipeline, mirroring the E-16..E-18 treatment. E-40 and E-41 are environment / artifact faults surfaced during construction; both raise `Kobako::SetupError`, with E-40 raised as the `Kobako::ModuleNotBuiltError` subclass.
