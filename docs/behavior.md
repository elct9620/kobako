...
| **Notes** | Regexp is a Rust capability gem composed into the Guest Binary shell, the pure-compute peer of the IO / Kernel surface (B-04); like every guest stdlib capability it carries no per-feature wire contract — the closed 12-entry wire type set already excludes `Regexp` / `MatchData`, so projecting a result to wire types is structural, not a new envelope. Coverage is an explicitly specified subset of the CRuby `Regexp` / `MatchData` API, not its full surface: there are no `Encoding` objects, and match offsets and substring slices (`#begin` / `#end` / `#offset`, `pre_match` / `post_match`, `captures`) are byte-based. The matching engine is an implementation choice below this contract; the Guest Binary backs it with a Rust gem, the same way the IO capability (B-04) is a Rust gem. |
