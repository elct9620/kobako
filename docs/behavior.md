...
| **Result / Final State** | The Service method's invocation terminates immediately as if it had `return`ed `val`. No code in the Service method body after the `yield` statement runs. When `val` is, or contains, a Capability Handle, it is restored to its original host object before the unwind (B-37). The Member call in the guest code (`Name::MemberName.method_name(...) { ... }`) returns `val`. Subsequent guest code runs normally; `break` does not terminate the enclosing guest method or invocation. |
