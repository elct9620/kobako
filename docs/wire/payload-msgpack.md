...
**The host's two outbound dispatch positions carry the bound themselves.** The Host Gem's codec library offers no depth limit on its encoder, so a Service's dispatch answer and the arguments it yields to a block are measured against the bound before that library writes them: a value nesting deeper — as a reference cycle, list or map, necessarily does — is refused where the Service hands it over, since the Service is the only side that can change it: an answer as the Service's own failure under fault `runtime` ([`CD-018`](../spec/behavior/codec.md)), a yield's arguments at the yield site, where the Service may rescue the refusal and, leaving it unrescued, fails the same way ([`T-159`](../spec/behavior/transport-yield.md)). Nothing past the bound reaches the library's walk, whose frames carry no stack guard, so the refusal is the same on every occurrence, on any thread.
