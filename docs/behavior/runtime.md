...
Concurrent-invocation isolation, the Guest Binary ABI gate, the warm `Kobako::Pool`, and the canonical boot state every invocation begins from. The governing summary lives in [`SPEC.md`](../../SPEC.md)
...
| Field | Value |
|-------|-------|
| **Initial State** | A `Kobako::Sandbox` constructed with a `gvl:` mode — `:hold` (the default) or `:release`. Distinct Threads may invoke concurrently, on distinct Sandboxes or a shared one (B-22). |
