...
| **Result / Final State** | Construction compares the runtime's declared profile against the requested floor. A declaration at or above the floor constructs normally; a declaration below it fails with `Kobako::SetupError` before any invocation entry point runs (E-49), mirroring the B-01 / B-40 setup-time treatment. The declaration is a contract statement, not a switch: the requested floor never alters runtime behavior — a `hermetic` runtime satisfies a `:permissive` floor by running exactly as it always does. The configured floor is readable via `Kobako::Sandbox#profile` alongside the cap readers; a `profile:` value outside the ladder — `nil` included, since the no-floor request is `:permissive` — is rejected at host pre-flight per E-39. |
