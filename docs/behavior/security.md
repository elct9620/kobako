...
## B-54 — Runtime declares its isolation profile; construction enforces the requested floor

| Field | Value |
|-------|-------|
| **Initial State** | A Host App constructs a `Kobako::Sandbox` over a runtime implementation. Every runtime declares exactly one isolation profile from the ordered ladder `permissive < hermetic`. `hermetic` declares the full ambient-denial posture: the WASI-boundary denial of ambient time and entropy (B-45) with no filesystem, environment, or network reachability through the WASI surface, and no host import beyond the wire ABI's closed set (whose single host import is `__kobako_dispatch`) — so the guest's only paths outward are injected Services (B-08) and the captured stdout / stderr write surface (B-04). `permissive` declares nothing beyond the Wasm cell. The bundled wasmtime runtime declares `hermetic`. |
| **Operation** | The Host App passes `profile:` to `Kobako::Sandbox.new` — `:permissive` or `:hermetic`, defaulting to `:hermetic` — naming the weakest posture it accepts. |
| **Result / Final State** | Construction compares the runtime's declared profile against the requested floor. A declaration at or above the floor constructs normally; a declaration below it fails with `Kobako::SetupError` before any invocation entry point runs (E-49), mirroring the B-01 / B-40 setup-time treatment. A declaration the Host Gem cannot place on the ladder is fail-closed: it ranks below every floor and fails construction identically. The declaration is a contract statement, not a switch: the requested floor never alters runtime behavior — a `hermetic` runtime satisfies a `:permissive` floor by running exactly as it always does. The configured floor is readable via `Kobako::Sandbox#profile` alongside the cap readers; a `profile:` value outside the ladder — `nil` included, since the no-floor request is `:permissive` — is rejected at host pre-flight per E-39. |
