...
| **Operation** | Guest code presents an arbitrary integer as a Handle id — as a Call `target`, or anywhere in a payload it encoded itself. A payload adapter kobako does not define carries no construction blocking (B-39) and no immutability (B-60), so the guest is free to name any number. |
...
| Field | Value |
|-------|-------|
| **Initial State** | A Sandbox executing mruby guest code. The Guest Binary provides `Regexp` and `MatchData` as guest-visible Ruby classes — a regexp literal (`/.../`), `Regexp.new` / `Regexp.compile`, and the `String` integration methods — that compile and run patterns against `String` values. Neither class is among the 12 wire types (→ [`docs/wire/payload-msgpack.md`](../wire/payload-msgpack.md) § Type Mapping). |
