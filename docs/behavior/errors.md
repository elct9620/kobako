...
| The Service supplied a yield argument with no wire representation | The Host App's Service | `runtime` | `Kobako::ServiceError` | `Kobako::ServiceError` | E-57 |
...
| E-05 | The guest fails to compile the source supplied to `#eval` before any execution begins; the message names where the parse stopped (`(eval):line:column:` ahead of the parser's diagnostic) | B-02 — fresh invocation |
...
| E-57 | Host Service method supplies a yield argument with no wire representation per [`docs/wire/payload-msgpack.md`](../wire/payload-msgpack.md) § Type Mapping — the outbound mirror of the guest's refusal at its own dispatch call site (E-55). Raised as `Kobako::YieldValueError` at the Service's `yield` site, where the Service may rescue it; the guest is never re-entered and the block never runs. It arises only where the yield surface accepts arbitrary host objects, which is what leaves the conversion to the boundary — a frontend whose yield surface is the wire type set refuses at the call site instead | `Kobako::ServiceError` | B-24 — yield round-trip |
...
| # | Trigger | Detection point | Raised class |
|---|---------|-----------------|--------------|
| E-32 | `#preload(code:)` source fails mruby compilation when a subsequent invocation replays the snippet | guest replay (first invocation) | `Kobako::SandboxError` (the message names where the parse stopped — `(snippet:Name):line:column:` ahead of the parser's diagnostic; compilation runs no snippet code, so the backtrace is empty) |
