...
| Field | Value |
|-------|-------|
| **Initial State** | A Sandbox executing mruby guest code. A Service is bound at `MyService::KV`. The guest holds a reference to the constant `MyService::KV` and calls a method on it. |
...
| **Operation** | Guest code calls a method directly on `handle_a` (e.g., `handle_a.find(id)`), using the Handle as the dispatch target. The wire layer encodes `handle_a` as the `target` field of the Call. |
...
| Field | Value |
|-------|-------|
| **Initial State** | Invocation N has completed. The guest code attempts to retain a Handle ID from invocation N and presents it as the `target` in a new invocation (N+1, of either verb). Invocation N+1 mints its own Catalog::Handles with the counter starting at 1, so no entry from invocation N (Service-returned via B-14 or host-injected via B-34) is reachable from it. |
...
## B-19 — Handle IDs are Sandbox-private: an ID from one Sandbox is undefined on another

| Field | Value |
|-------|-------|
| **Initial State** | Two `Kobako::Sandbox` instances have each completed one or more invocations, every invocation having minted its own Catalog::Handles (B-18). |
| **Operation** | A Handle ID issued on Sandbox A is presented on Sandbox B — as a dispatch `target` or as a call argument — or Sandbox A is garbage-collected. |
| **Result / Final State** | Handle IDs are not a process-global namespace: Sandbox B resolves the foreign ID against its own invocation's table alone, so the dispatch is refused as an undefined target and answers with a fault Reply instead of reaching Sandbox A's object. No entry is shared with, transferred to, or reachable from another Sandbox instance, and none outlives the Sandbox that issued it — discarding a Sandbox releases every table its invocations produced. Handles are not reference-counted and there is no explicit `release` API for individual entries; validity is scoped to the issuing invocation (B-18) and bounded by its Sandbox, uniformly across allocation sources (B-14 / B-34). |
...
| **Result / Final State** | A bare integer never becomes a capability on either side. On the host, no public path constructs a Handle, and an instance fabricated through a non-public path and passed to `#run` raises `ArgumentError` at host pre-flight (E-29). Inside the guest, `Kobako::Handle` construction is blocked (B-39) and a held Handle is frozen (B-60), so the bundled guest can present only ids of Handles it actually holds. The boundary is neither the wire shape nor ID secrecy: a Call carries a Handle `target` as a bare id its sender chooses, so the value reaches the host and the invocation's `Catalog::Handles` membership answers it — an id the table never issued is refused as `undefined` (E-13), resting on membership plus per-invocation isolation (B-18). That guarantee holds for any sender (B-65); in-guest unforgeability narrows the guess/probe surface within the bundled guest as defense-in-depth. A guessed or forged ID grants no reference the invocation was not already handed (B-14 / B-34). |
...
| Field | Value |
|-------|-------|
| **Initial State** | `#run(target, *args, **kwargs)` is invoked (B-31). At least one element of `args`, or at least one value in `kwargs`, is not wire-representable per the type set defined in B-13 — for example, a `StringIO`, an arbitrary Host App `Env` instance, or any other Ruby object whose class is outside the wire 11-entry mapping. |
| **Operation** | While encoding the Run's payload the Host Gem walks the `args` Array and the `kwargs` Hash values; container types (Array, Hash) are walked one level at a time, and each leaf value that is not wire-representable is allocated into the Sandbox's Catalog::Handles. The allocator returns a fresh u32 ID (B-15) which is written into the payload as an `ext 0x01` Capability Handle in place of the original Ruby object. Wire-representable leaves pass through unchanged. The walk wraps Hash values only: a Hash key must already be wire-representable (B-13), since a key cannot cross as a Capability Handle the way a value can, so a non-representable key is rejected with `Kobako::SandboxError` rather than auto-wrapped. |
| **Result / Final State** | The guest mruby code receives a `Kobako::Handle` proxy at each position where the host supplied a non-wire-representable argument. The proxy carries no observable Ruby value content; method calls on it dispatch back to the host through the same `method_missing` → Transport path the guest uses for Service-returned Handles (B-17). The host-side `Catalog::Handles` entry remains live for the duration of the invocation and, like every other Handle in that run's table, is unreachable from any later invocation (B-18 / B-19). `Catalog::Handles` cap exhaustion during the walk raises `Kobako::HandleExhaustedError` at host pre-call via the same path as B-21 / E-07. The walk traverses Array and Hash containers but does not descend into the instance variables or internal structure of a non-wire-representable leaf — once a leaf is wrapped, its sub-structure is hidden behind the Handle. A `Kobako::Handle` already produced internally by the Host Gem is rejected at host pre-flight (E-29); auto-wrap never re-wraps an existing Handle. |
