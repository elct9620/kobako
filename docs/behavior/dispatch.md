...
| **Result / Final State** | A bare integer never becomes a capability on either side. On the host, neither the guest→host wire nor the Host App API turns an integer into a working Handle: a raw integer presented as a dispatch target does not carry the Handle wire encoding (`ext 0x01`), so the host-side wire decoder rejects the malformed encoding before the value reaches `Catalog::Handles`, and a `Kobako::Handle` instance fabricated on the host side through any non-public path and passed to `#run` raises `ArgumentError` at host pre-flight (E-29). Inside the guest, `Kobako::Handle` construction is blocked (B-39) and a held Handle is frozen (B-60), so the guest cannot construct, re-point, or forge an id — it can present only ids of Handles it actually holds. The `Kobako::Handle` class holds the u32 ID internally and exposes no public reader for it. ID secrecy is still not the primary boundary — the host's per-invocation `Catalog::Handles` `fetch` resolves only ids the invocation was already handed and rejects the rest (E-13), resting on membership plus per-invocation isolation (B-18), not on ID secrecy — but in-guest unforgeability (B-60) closes the guess/probe surface as defense-in-depth, so a guessed or forged ID grants no reference the invocation was not already handed (B-14 / B-34). |
