...
| **Operation** | `execution = sandbox.eval(code)` (or `sandbox.run(target, *args, **kwargs)`), then any reads on the returned object: `#value`, `#failed?`, `#stdout`, `#stderr`, `#stdout_truncated?`, `#stderr_truncated?`, `#usage`. |
| **Result / Final State** | A successful invocation returns a frozen `Kobako::Execution` — a value object bundling that one run's `#value` (the deserialized guest result: the last `#eval` expression per B-06, or the `#run` entrypoint's return per B-31), its `#stdout` / `#stderr` captures with the matching `#stdout_truncated?` / `#stderr_truncated?` predicates (B-04), and its `#usage` (B-35). The object is frozen at return and never mutates; each invocation produces a separate `Execution`, so a later run never alters an earlier one. A failed invocation raises one of the three invocation-outcome errors (`Kobako::TrapError` / `Kobako::SandboxError` / `Kobako::ServiceError`) carrying the same frozen `Execution` on the error's `#execution`, so a rescue reads that run's captures and usage exactly as a successful caller reads them off the return value; on a failed run `#value` is `nil` and only the captures and usage are meaningful. `#failed?` is `true` iff the run failed — `false` on the Execution a successful invocation returned, `true` on the one a raised error carries — so a run whose value was legitimately `nil` stays distinguishable from a failed one from the object alone. It reports only that the run failed, never with what: the Execution exposes no reader for the error that carries it, so a rescue reads the failure off the error and the observables off the Execution. A pre-flight failure that enters no invocation — an input rejected before the guest runs, such as a non-String `code` or a malformed `#run` target — raises with `#execution` left `nil`, since no run produced observables. The reusable Sandbox exposes no capture or usage readers of its own; every observable lives on the run's `Execution`. |
