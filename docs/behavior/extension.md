...
| Field | Value |
|-------|-------|
| **Initial State** | A Sandbox with one or more installed Extensions carrying a `backend`, each declared as a fixed object (`object:`), a per-invocation callable (`provider:`), or a fillable carrying neither. |
| **Operation** | An invocation (`#eval` or `#run`) begins. |
| **Result / Final State** | Each backend's kind is chosen by which keyword it declares — never inferred from whether the value is callable — and contributes the path's object for this invocation accordingly. A **static** backend (`object:`, or a bare positional object) is that object, fixed once and identical across every invocation. A **per-invocation** backend (`provider:`, a no-argument callable) has its provider invoked once at the start of each invocation, yielding that invocation's object, so a fresh object backs the path on every invocation and no writable per-invocation state leaks across invocations (B-03); each invocation resolves its own object, so concurrent invocations on the same Sandbox never share it. A **fillable** backend defaults to the `Kobako::Unresolved` sentinel until the host supplies the invocation's object (B-62). Declaring both `object:` and `provider:` raises `ArgumentError`. Provider identity is resource identity — distinct `provider:` values resolve to distinct objects, and one provider value shared by several Extensions resolves once per invocation to a single object shared by their paths. The bound path set is fixed at the seal and its Frame 1 preamble is unchanged across invocations (B-33); only the object chosen behind each path varies per invocation. A `provider:` callable that raises propagates its exception unchanged to the `#eval` / `#run` caller — never a wrapped `Kobako` error — and the guest does not run; resolution being per-invocation, a later invocation whose provider succeeds runs normally. |
