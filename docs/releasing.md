...
The gem `.` package carries **no `component`** on purpose: with `include-component-in-tag: false` its release-PR section is componentless, and release-please tags a standalone (gem-only) release only when the configured component matches that — naming the component makes a gem-only release silently create no tag. Without a component the Ruby strategy cannot derive where `VERSION` lives, so the package names `version-file` itself; dropping it leaves `lib/kobako/version.rb` behind while the tag and CHANGELOG move on, and the release builds the previous version. `package-name` would locate the file too, but it sets the component and brings back the missing tag.
...
| `feat:` | minor |
| `fix:` | patch |
| any type carrying `!` or a `BREAKING CHANGE:` footer | major |
| unmarked `refactor` `docs` `test` `chore` `build` `ci` `perf` `style` | none — non-releasing |

An **unmarked** `refactor`/`docs`/`test` commit never releases: a change shipped only under those types is invisible to `release-please`. The breaking marker overrides the type — `release-please` reads a commit's breaking flag without consulting its type, so a `refactor!` both releases and reaches the changelog where a plain `refactor` does neither.
...
| Catch-up: real change landed only as unmarked `refactor`/`docs`/`test` (e.g. behavior shipped inside the bundled `kobako.wasm`) | Give it a trigger. **Preferred:** land a genuine `feat`/`fix` touching that track's paths (correcting a stale README example is a real `fix`). **Fallback (no natural feat/fix exists):** a `Release-As: X.Y.Z` footer — see rules below. |
...
| A change is breaking | See the rules below. |

## Marking a breaking change

The marker is the whole of what a reader upgrading across the release is handed, so these three rules decide whether that reader has a migration path at all.

| Rule | Why |
|------|-----|
| Put the marker on the track whose **public surface** actually broke | A change can break one track and not the other: the `Kobako::Member` → `Kobako::Proxy` guest change broke gem-user scripts but left the crate Rust API intact — the gem for a guest-Ruby idiom (guest scripts are gem-user-facing), a crate for its Rust API. |
| Carry the migration in a `BREAKING CHANGE:` footer, naming the old spelling and the new | With `!` alone the changelog's note **is the commit subject** — the body is never read, however completely it explains the rename. A subject that names neither spelling leaves the reader nothing to act on. |
| Keep a breaking commit inside **one** track's paths | `exclude-paths` scopes the commit, not the marker. A commit touching both root and `wasm/`/`crates/` announces its break on both tracks, so a crate-only rename reads to a gem user as a gem break. |
...
| Read the generated **⚠ BREAKING CHANGES** section and confirm a reader could act on every line | The last point at which a subject-only note can be given its migration text. Rewriting the section in the PR is safe as long as nothing else lands afterwards — `release-please` regenerates the branch on every push. |
