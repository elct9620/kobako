...
- **Host-allocated**: the wire layer on the host side allocates a Handle automatically in two symmetric situations — whenever a Service method returns a stateful object (host→guest return path, → [`T-001`](spec/behavior/transport-dispatch.md)), and whenever `#run` is invoked with arguments containing non-wire-representable objects (host→guest argument path, → [`T-066`](spec/behavior/transport-dispatch.md)). The Host App has no API to create or inspect Handles directly.
...
- **No reachable un-delivered Handle**: a Handle ID names an object only for as long as the invocation that minted it, and every ID that invocation minted was handed to the guest in the same message that minted it. A guest naming an arbitrary integer as a Handle therefore reaches either an object it already holds or nothing at all ([`T-017`](spec/behavior/transport-dispatch.md)). This is a property of the table's lifetime, not of an enumeration of allocation sites: the table is minted per invocation, its IDs ascend from 1, and it is discarded whole when the invocation ends — so an invocation that fails part-way through minting takes its un-delivered IDs with it.
...
| `"argument"` | A Service method's argument binding failed — an unknown keyword, or an arity mismatch ([`T-144`](spec/behavior/transport-dispatch.md), [`T-141`](spec/behavior/transport-dispatch.md)) |
...
| `"internal"` | The exchange failed with no Service outcome to report: the request was unreadable ([`T-043`](spec/behavior/transport-dispatch.md)), or the host could not issue the Handle the answer needs ([`T-046`](spec/behavior/transport-dispatch.md)). Separate from `"runtime"` because retrying the same call against a working host would behave differently, which is not true of a Service that raised |
| `"block"` | The caller's own block failed while the Service was yielding to it, and the Service did not rescue it ([`T-095`](spec/behavior/transport-yield.md)). The failure is the caller's, so it carries no host detail: a caller holding the exception it raised continues that one, and a caller whose block value was refused instead raises under the class the message names |
...
When a Service method invokes `yield` or `block.call` ([`T-085`](spec/behavior/transport-yield.md)) on the Yielder materialised from a Call with `block_given=true`, the Host Gem re-enters the Guest Binary synchronously to execute the block body. This is the reverse-direction Call/Reply pair: the host issues the Call, the guest answers with the Reply.
...
- **Scope**: a Yielder is valid only for the duration of the dispatch frame that produced it. Invoking it after that frame returns raises ([`T-136`](spec/behavior/transport-yield.md)).
- **Nesting**: dispatch frames stack in LIFO order; each frame holds at most one Yielder, and nested frames have independent Yielders ([`T-092`](spec/behavior/transport-yield.md)). The wasm stack budget bounds nesting depth.
...
| `0x02` | **break** | wire-legal value | The block executed `break val` from a non-lambda, non-orphan context. The host yield site terminates the Service method's invocation with `payload` as the effective return value ([`T-089`](spec/behavior/transport-yield.md)). |
| `0x03` | RESERVED | — | Reserved tag value. Either endpoint rejects this tag as a wire violation. |
| `0x04` | **error** | Error Record `{class, message, backtrace}` | The block raised an exception, returned a value with no wire representation ([`T-135`](spec/behavior/transport-yield.md)), used `return` from a non-lambda block ([`T-134`](spec/behavior/transport-yield.md)), or invoked an escaped Yielder ([`T-136`](spec/behavior/transport-yield.md)). The host yield site re-raises a Ruby exception with the named class and message. |

The `0x01` ok payload follows the same type mapping as any Reply success value (→ [`docs/wire/payload-msgpack.md`](wire/payload-msgpack.md) § Type Mapping). Capability Handle references (ext 0x01) are legal in the payload position; because host code consumes the ok value, the host restores them to their original objects before the `yield` expression returns (→ [`T-100`](spec/behavior/transport-yield.md)).

The `0x02` break payload carries the value supplied to `break`. The Host Gem unwinds the Service method's invocation, presenting `payload` to the guest dispatch site as the Service method's return value. A Capability Handle here is **not** restored — the value returns to the guest, not to host code, so it rides back unchanged on the same ID ([`T-102`](spec/behavior/transport-yield.md)).
...
The Wire Spec is pinned by a single u32 ABI version (→ [`docs/wire-codec.md`](wire-codec.md) § ABI Version): the Guest Binary reports the version it was built against via `__kobako_abi_version`, and the Host Gem accepts it at Sandbox construction only on equality ([`RT-006`](spec/behavior/runtime.md), [`RT-007`](spec/behavior/runtime.md)). A running sandbox is short-lived (instantiated per invocation, retired after the outcome is retrieved), so there are no long-lived cross-version connections and no stored wire payloads that outlast an ABI version.
