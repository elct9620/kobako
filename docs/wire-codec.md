...
Encoded values nest to at most 128 levels — the MessagePack ecosystem's established limit, which the Host Gem's codec library already enforces on decode. The Guest Binary encoder caps its recursive walk at the same depth, so it never emits a structure the host cannot decode and never overflows the wasm stack: a guest return or yield-block result nesting deeper than the bound — which a reference cycle necessarily does — has no wire representation and surfaces as E-06 / E-22 (→ [`behavior.md`](behavior.md)) rather than a hard trap. A `#run` argument that nests deeper is encoded best-effort — the over-deep subtree is rendered through `Object#to_s` at the cap, the same fallback an unknown argument type takes — so the dispatch stays bounded rather than trapping.
