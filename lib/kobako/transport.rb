...
module Kobako
  ...
  # +YieldProxy+ factory. +Sandbox#initialize+ wires them onto the
  # +Runtime+ as a dispatch +Proc+ + +yield_to_guest+ lambda pair
  # ({BRIDGE_REDESIGN §5.5.3}); there is no separate Channel object.
  # Replaces the former +Kobako::RPC+ namespace; "RPC" implies a
  # cross-process boundary that kobako does not have — host and guest
