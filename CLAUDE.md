...
- **mruby C API FFI surface** — `wasm/kobako-mruby-sys/src/wrapper.h` + `build.rs::run_bindgen` produce the FFI surface from `vendor/mruby/include/` at build time (bindgen scoped to this crate so libclang stays a sys-only build dep). `src/lib.rs` `include!`s the generated `bindings.rs` and adds the layout-pin assertions on `mrb_value`. `src/{bytecode,io,value}.c` are the three remaining layout-safe C shims (IREP parse + classify, `mrb_obj_as_string` + `fwrite` loop, and a single float unboxer that sidesteps a `union mrb_value_` ABI mismatch under `MRB_WORDBOX_NO_INLINE_FLOAT`). `src/{state,value,class,ccontext,array,hash}.rs` carry the safe layer (`Mrb` RAII, typed `Value` / `Class` / `Array` / `Hash` newtypes, `Ccontext` RAII). The crate's `build.rs` owns `libmruby.a` linkage (`MRUBY_LIB_DIR`) and the wasi-sdk setjmp wiring (`WASI_SDK_PATH`); `kobako-wasm` consumes the crate via the `crate::mruby` façade (`crate::mruby::sys`, `crate::mruby::Mrb`, `crate::mruby::sys::Value`, `crate::mruby::sys::Class`).
