...
| HandleTable / capability handles | `lib/kobako/handle_table.rb` | B-13..B-21 in `docs/behavior.md`. Owned by Sandbox (B-19), injected into `Kobako::RPC::Server` so guest→host dispatch and host→guest wire encoding share one allocator. Per-invocation reset is the Sandbox's job — Server holds the reference but never calls `#reset!`. |
| Service registration | `lib/kobako/rpc/server.rb`, `lib/kobako/rpc/namespace.rb` | Per-Sandbox Server owns the Namespace registry; bound objects live one level deep at `"Namespace::Member"`. HandleTable is injected by the owning Sandbox, not owned by Server. |
