...
1. **SPEC.md is the source of truth, and the glossary is the vocabulary it is written in.** Concepts are declared in `docs/spec/_data/glossary.yml` and rendered by `rake spec:generate` — edit the data file, never the page. The glossary carries concepts alone: how one is spelled in Ruby or Rust, and which anchors govern what it does, are statements about an implementation and belong where those are specified. A name it rules out must appear zero times in the corpus, so two words that both name concepts of ours (`Execution` and `Invocation`) are told apart by their definitions rather than by a gate. Behavior contracts live in `SPEC.md` or in the `docs/<topic>.md` it indexes; the per-anchor behavior detail lives in `docs/behavior/<aspect>.md`, mapped from each anchor range by the grouping table in `SPEC.md` § Behavior. Behavior anchors form append-only families that are never renumbered; the `rake gate:anchors` gate enforces uniqueness, contiguity, and resolvability across the spec corpus — consult the grouping table for the current families and ceilings. **Cite anchors only where traceability belongs: in tests (the test that verifies an anchor names it) and in prose docs (linking directly to the behavior file that holds it). Implementation and public-API comments state intent — what + why — not anchor citations.** When SPEC is silent, extend it (or the relevant `docs/<topic>.md`) first.
...
| Render the generated spec pages after editing `docs/spec/_data/` | `rake spec:generate` |
...
| Vocabulary / what a concept is called | `docs/spec/_data/glossary.yml` (page: `docs/spec/glossary.md`) | Reader: `tasks/support/spec/glossary.rb`, gated by `gate:spec:glossary`. Whether a word the corpus uses is a missing concept is a reading, not a gate — no lexical rule separates `Fault` from `Symbol`. |
