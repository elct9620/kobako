...
1. **SPEC.md is the source of truth, and the glossary is the vocabulary it is written in.** Concepts are declared in `.spec/glossary.json` and rendered to `docs/spec/glossary.md` by `sumi render` — edit the declaration, never the page. The glossary carries concepts alone: how one is spelled in Ruby or Rust, and which anchors govern what it does, are statements about an implementation and belong where those are specified. A name it rules out is reported by `sumi verify` at every line that uses it, so ruling one out is a claim about wording to be fixed rather than a fact about the corpus to be waited for; two words that both name concepts of ours (`Execution` and `Invocation`) are still told apart by their definitions, since rejecting either would flag every legitimate use of the other. Behavior contracts live in `SPEC.md` or in the `docs/<topic>.md` it indexes; the per-anchor behavior detail lives in `docs/behavior/<aspect>.md`, mapped from each anchor range by the grouping table in `SPEC.md` § Behavior. Behavior anchors form append-only families that are never renumbered; the `rake gate:anchors` gate enforces uniqueness, contiguity, and resolvability across the spec corpus — consult the grouping table for the current families and ceilings. **Cite anchors only where traceability belongs: in tests (the test that verifies an anchor names it) and in prose docs (linking directly to the behavior file that holds it). Implementation and public-API comments state intent — what + why — not anchor citations.** When SPEC is silent, extend it (or the relevant `docs/<topic>.md`) first.
...
| Render the spec pages after editing `.spec/`, and check the source against them | `sumi render` · `sumi verify` |
...
| Vocabulary / what a concept is called | `.spec/glossary.json` (page: `docs/spec/glossary.md`, written by `sumi render`) | A vocabulary naming no subdomain is Global; a named one scopes its own definitions to its `include` globs, and later entries override earlier ones outright. sumi reports rejected names against comments in `.rb` / `.rs` and against any other file entire — so `include` stays off append-only files, whose shifting line numbers would strand an `ignore`. What is checked here instead is what needs no corpus: `tasks/support/spec/glossary.rb`, gated by `gate:spec:glossary`. Whether a word the corpus uses is a missing concept is a reading, not a gate — no lexical rule separates `Fault` from `Symbol`. |
...
| Which assembly level an outside consumer stands on | `docs/architecture.md` | The four-level ladder and what each level fixes for you. Reader-facing navigation, not a contract — obligations stay in `customization.md`, anchors in SPEC, and this file's Layering section keeps the in-repo view. |
