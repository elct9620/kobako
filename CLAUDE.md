...
2. **One thing per file; keep files small.** When a module grows, split it into a façade plus per-responsibility files in a sibling directory. `Kobako::RPC` and `Kobako::Snippet` follow this pattern (`<name>.rb` façade + `<name>/` directory of focused files — e.g. `rpc/{server,dispatcher,envelope,fault,handle,handle_table,namespace}.rb`, `snippet/{binary,source,table}.rb`). Prefer adding a new file over expanding an existing one.

   **Types nest under a Module, not a Class.** A stateful Class is per-instance and should not double as the namespace for sibling types. Place new types at the top level (`Kobako::Capture`, `Kobako::Invocation`) or under a Module (`Kobako::RPC::Envelope::Request`, `Kobako::Outcome::Panic`, `Kobako::Snippet::Table`). Do not introduce Classes nested under a Class for type-grouping purposes.
...
4. **Follow language community conventions via tooling.** Ruby: Rubocop (auto-applied on `.rb` Edit/Write via PostToolUse hook) plus Steep (`bundle exec steep check` runs on every `.rb` / `.rbs` Edit/Write — failures block the edit). Rust: `cargo fmt` and `cargo clippy` (both auto-applied on `.rs` Edit/Write via PostToolUse hook; clippy runs with `-D warnings`, and uses `--target wasm32-wasip1` for `wasm/*` manifests when the cross-toolchain is provisioned). When a cop or lint fires, **shrink the code to fit the tool** — don't widen `.rubocop.yml` exclusions or add `#[allow]` / `# steep:ignore`. Existing exclusions (test files for `Metrics/ClassLength` / `Metrics/AbcSize`, `tasks/vendor.rake` and `*.gemspec` for `Metrics/BlockLength`, `test/support/wire_value_generator.rb` for `Naming/VariableNumber`) are anchored to specific SPEC-to-code mappings; add new ones only with an inline comment naming the mapping.

   **Tool-vs-tool conflicts are the one justified widening.** When a Rubocop cop and Steep / RBS upstream disagree on the same code shape — `Style/DataInheritance` rejects `class X < Data.define(...)` while ruby/rbs ([`docs/data_and_struct.md`](https://github.com/ruby/rbs/blob/master/docs/data_and_struct.md)) documents the subclass form as the Steep-friendly pattern — prefer the type-system guidance and disable the cop at the `.rubocop.yml` level with a comment citing the upstream source. `Style/DataInheritance` is disabled in `.rubocop.yml` on that basis. `Kobako::Outcome::Panic`, `Kobako::Invocation`, `Kobako::SandboxOptions`, and the `Kobako::Snippet::{Source,Binary}` pair are the migrated examples: subclass form, no `# steep:ignore` tax, constants in the class body. The four `Data.define` types still on the assignment form in `lib/kobako/rpc/**` (`Handle`, `Envelope::Request`, `Envelope::Response`, `Fault`) are transitional debt — migrate them when touching nearby code; do not write new types in the assignment form.
...
10. **Test assertion messages are contract statements, not implementation narrative.** The `assert_*` message renders at failure-read time to a contributor whose primary need is "which contract did this change violate" — not "which crate-private branch took the wrong path." Leaking internal names (`RStringEmbed.ary`, `as_.heap.ptr`, `mrb_obj_as_string coercion arm`, `write(2)` syscall) into the message turns the failure report into noise for anyone without the header context, and rots faster than the test source does when internals get renamed. Phrase each message as "<input shape> through <public API> must <observable behaviour>"; keep the witness rationale (why this boundary value, why this branch is the load-bearing case) in the comment block above the test method. The IO write coverage tests in `test/test_e2e_journeys.rb` are the worked correction example.
...
- **Wire format / codec** — `lib/kobako/codec/` (host byte codec) + `lib/kobako/rpc/envelope.rb` (host RPC Request / Response envelope on top of the codec) + `wasm/kobako-wasm/src/codec/`, `wasm/kobako-wasm/src/rpc/` (guest). Abstract envelope shapes live in `docs/wire-contract.md`; byte-level reference in `docs/wire-codec.md`; `SPEC.md` retains the governing summary for both.
- **Error taxonomy / outcome attribution** — `lib/kobako/errors.rb` + `lib/kobako/outcome.rb` + `lib/kobako/outcome/panic.rb`. Anchors: E-xx in `docs/behavior.md`; the governing summary lives in `SPEC.md` § Behavior.
- **Sandbox lifecycle / per-invocation flow** — `lib/kobako/sandbox.rb` (façade with `#eval` / `#run` / `#preload` / `#define`) + `lib/kobako/invocation.rb` (host pre-flight value object + invocation envelope encoder for `#run`; sits at top level rather than under `RPC::` because `#run` is host→guest dispatch, not part of the guest→host RPC channel) + `lib/kobako/sandbox_options.rb` (timeout / memory_limit normalisers) + `lib/kobako/snippet/table.rb` (per-Sandbox preloaded snippet registry; encodes Frame 3) + `lib/kobako/snippet/{source,binary}.rb` (per-entry value objects) + `lib/kobako/capture.rb` + `ext/kobako/src/wasm.rs` (host orchestration) + `wasm/kobako-wasm/src/abi.rs` (guest entry — `__kobako_eval` / `__kobako_run` / `__kobako_alloc` / `__kobako_take_outcome` exports). B-xx anchors live in `docs/behavior.md`.
- **Guest IO / `$stdout` / `$stderr`** — `wasm/kobako-wasm/src/kobako/io.rs` (top-level `::IO` class + `#write` / `#fileno` C bridges through `kobako_io_fwrite` C shim in `wasm/kobako-mruby-sys/src/io.c`) + `wasm/kobako-wasm/mrblib/io.rb` (instance method surface — `#print`, `#puts`, `#printf`, `#p`, `#<<`, `#tty?`, `#sync`, `#flush`, `#closed?`) + `wasm/kobako-wasm/mrblib/kernel.rb` (Kernel delegators routing through assignable `$stdout` / `$stderr`). Both mrblib files are precompiled by `wasm/kobako-wasm/build.rs` via host `mrbc` and embedded as RITE bytecode in the cdylib (see `wasm/kobako-wasm/src/kobako/bytecode.rs`). SPEC anchor: B-04.
- **RPC dispatch** — `lib/kobako/rpc/dispatcher.rb` (host; **never raises** — every failure becomes a `Response.err` envelope) + `wasm/kobako-wasm/src/rpc/` (guest).
- **HandleTable / capability handles** — `lib/kobako/rpc/handle_table.rb` + `lib/kobako/rpc/handle.rb`. Anchors: B-13..B-21 in `docs/behavior.md`.
- **Service registration** — `lib/kobako/rpc/server.rb` (per-Sandbox host coordinator owning the Namespace registry + HandleTable; exposed on `Sandbox` as `#services` / `#define`) + `lib/kobako/rpc/namespace.rb` (constant-named member buckets — bound objects live one level deep at `"Namespace::Member"`).
- **ABI surface (host ↔ guest exports)** — `wasm/kobako-wasm/src/abi.rs` + matching `ext/kobako/src/wasm.rs` callers.
- **E2E coverage / real guest binary** — `test/test_e2e_journeys.rb` drives `Sandbox#eval` (one-shot source) and `test/test_sandbox_run.rb` drives `Sandbox#run` (entrypoint dispatch) against `data/kobako.wasm`; together they are the load-bearing coverage for Wire ABI export presence and the host↔guest round-trip. Wrapper-tier tests (`test/test_wasm_wrapper.rb`) cover only the `from_path` pipeline and intentionally do not duplicate this — a missing export fails the first E2E run, so adding a wrapper-tier smoke test is redundant.
- **Build / toolchain** — `tasks/{vendor,mruby,wasm}.rake`.
- **mruby C API FFI surface** — `wasm/kobako-mruby-sys/src/wrapper.h` + `build.rs::run_bindgen` produce the FFI surface from `vendor/mruby/include/` at build time (bindgen scoped to this crate so libclang stays a sys-only build dep). `src/lib.rs` `include!`s the generated `bindings.rs` into a private `bindings` module (with `#[allow(clippy::all)]` containing bindgen's bitfield-accessor `transmute` patterns, per the rb-sys / rusty_v8 convention) and re-exports via `pub use bindings::*`. `wrapper.h` also carries `static inline` wrappers for the mruby macros and unexported helpers bindgen cannot reach (`RSTRING_PTR` / `RSTRING_LEN`, `mrb_obj_ptr`, `mrb_gc_arena_save` / `_restore`, `mrb_proc_new`); bindgen's `wrap_static_fns` emits a single `mruby_static_wrappers.c` trampoline file in OUT_DIR — the entire C translation unit the crate compiles. **No hand-written `.c` shims remain.** `src/{state,value,class,ccontext,array,hash}.rs` carry the safe layer (`Mrb` RAII, typed `Value` / `Class` / `Array` / `Hash` newtypes, `Ccontext` RAII). `build.rs` owns `libmruby.a` linkage (`MRUBY_LIB_DIR`) and the wasi-sdk setjmp wiring (`WASI_SDK_PATH`); `kobako-wasm` consumes the crate via the `crate::mruby` façade (`crate::mruby::sys`, `crate::mruby::Mrb`, `crate::mruby::sys::Value`, `crate::mruby::sys::Class`).
- **RBS type signatures** — `sig/kobako/` mirrors `lib/kobako/` one-to-one (façade `.rbs` + sibling directory for split classes). Three sources stack on top of `Steepfile`: (1) `sig/_external/` holds hand-written stubs for third-party gems that lack public RBS; (2) `rbs_collection.yaml` + `rbs_collection.lock.yaml` pin gem signatures from gem_rbs_collection; (3) Ruby stdlib stubs ship with the `rbs` gem and are opted into per-target with `library "<name>"` in `Steepfile` (e.g. `library "singleton"`, `library "forwardable"`). When adding a stdlib dependency, reach for the `library` directive first — do not hand-roll a stub in `sig/_external/`. `Steepfile` checks `lib` against `sig`. When adding a Ruby class, write its `.rbs` alongside — the PostToolUse steep hook will block the edit otherwise.
