...
- **Sandbox lifecycle / per-invocation flow** — `lib/kobako/sandbox.rb` (façade with `#eval` / `#run` / `#preload` / `#define`) + `lib/kobako/run_envelope.rb` (host pre-flight + invocation envelope encoder for `#run`) + `lib/kobako/sandbox_options.rb` (timeout / memory_limit normalisers) + `lib/kobako/snippet_table.rb` (per-Sandbox preloaded snippet registry; encodes Frame 3) + `lib/kobako/capture.rb` + `ext/kobako/src/wasm.rs` (host orchestration) + `wasm/kobako-wasm/src/abi.rs` (guest entry — `__kobako_eval` / `__kobako_run` / `__kobako_alloc` / `__kobako_take_outcome` exports). B-xx anchors live in `docs/behavior.md`.
