...
- **Guest IO / `$stdout` / `$stderr`** — `wasm/kobako-wasm/src/kobako/io.rs` (top-level `::IO` class + `#write` / `#fileno` C bridges through `kobako_io_fwrite` C shim in `wasm/kobako-wasm/src/mruby/io.c`) + `wasm/kobako-wasm/mrblib/io.rb` (instance method surface — `#print`, `#puts`, `#printf`, `#p`, `#<<`, `#tty?`, `#sync`, `#flush`, `#closed?`) + `wasm/kobako-wasm/mrblib/kernel.rb` (Kernel delegators routing through assignable `$stdout` / `$stderr`). Both mrblib files are precompiled by `build.rs` via host `mrbc` and embedded as RITE bytecode in the cdylib (see `wasm/kobako-wasm/src/kobako/bytecode.rs`). SPEC anchor: B-04.
