...
- **mruby C API FFI surface** — `wasm/kobako-mruby-sys/src/lib.rs` (hand-rolled `extern "C"` declarations + `mrb_state` / `mrb_value` ABI types) + `wasm/kobako-mruby-sys/src/{bytecode,exc,io,value}.c` (the four layout-safe C shims wrapping mruby's macro-only APIs) + `wasm/kobako-mruby-sys/src/{state,value,class,ccontext}.rs` (the safe layer: `Mrb` RAII, typed `Value` / `Class` newtypes, `Ccontext` RAII). The crate's `build.rs` owns `libmruby.a` linkage (`MRUBY_LIB_DIR`) and the wasi-sdk setjmp wiring (`WASI_SDK_PATH`); `kobako-wasm` consumes the crate via the `crate::mruby` façade (`crate::mruby::sys`, `crate::mruby::Mrb`, `crate::mruby::sys::Value`, `crate::mruby::sys::Class`). Future bindgen migration is scoped to this crate.
