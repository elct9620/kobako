...
- **Wire format / codec** — `lib/kobako/codec/` (host byte codec) + `lib/kobako/wire/` (host RPC envelope on top of the codec) + `wasm/kobako-wasm/src/codec/`, `wasm/kobako-wasm/src/envelope.rs` (guest). Byte-level reference lives in `docs/wire-codec.md`; `SPEC.md` § Wire Codec retains the governing summary.
