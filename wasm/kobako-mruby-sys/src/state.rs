...
impl Mrb {
    ...
    // ----------------------------------------------------------------
    // Argument unpack. Each `get_args_*` method wraps a specific
    // `mrb_get_args` format string used by the bridge layer:
    //
    //   - get_args_o        — "o"  → single positional
    //   - get_args_rest     — "*"  → rest array
    //   - get_args_n_rest   — "n*" → symbol + rest array
    //   - get_args_io       — "io" → integer + object
    //
    // The rest-form variants borrow the call frame's argv slice; the
    // lifetime is tied to `&self`, which the bridge body holds for
    // the duration of the C call. mruby may set the rest pointer to
    // NULL when the rest count is zero — the helpers fold that into
    ...
    pub fn get_args_o(&self) -> Value {
        ...
        // SAFETY: `self.state` is alive by the `&self` borrow;
        // `&mut raw` is a valid `*mut mrb_value`; the `"o"` format
        // string writes exactly one cell.
        unsafe {
            sys::mrb_get_args(
                self.as_ptr(),
                crate::cstr!("o"),
    ...
    /// a borrowed slice into the call frame.
    #[cfg(target_arch = "wasm32")]
    ...
    pub fn load_irep_buf(&self, bytes: &[u8]) -> Value {
        // SAFETY: `self.state` is alive; `bytes` is borrowed for the
        // synchronous call.
        Value::from_raw(unsafe {
